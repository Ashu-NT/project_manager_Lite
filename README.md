...
- `Maintenance Management` as an available early-phase module with persisted master/work execution foundations, sensor/integration monitoring, preventive planning support, planner workbench support with preventive readiness queues, first technician-facing work-order execution actions for step start/done/confirm plus task completion, linked execution evidence visibility, reliability analytics/reporting, and first shell workspaces for `Maintenance Dashboard`, `Locations`, `Systems`, `Task Templates`, `Asset Library`, `Preventive Plan Library`, `Assets`, `Sensors`, `Requests`, `Work Orders`, `Documents`, `Preventive Plans`, `Planner`, and `Reliability`
