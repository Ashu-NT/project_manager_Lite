...
def test_cost_edit_action_passes_expected_version(monkeypatch):
    ...
    class _Probe(CostActionsMixin):
        ...
        def reload_costs(self, preferred_cost_id=None):
            _ = preferred_cost_id
            self.reloaded = True
...
def test_task_edit_action_recovers_from_stale_write(monkeypatch):
    ...
    monkeypatch.setattr("ui.modules.project_management.task.actions.TaskEditDialog", _Dialog)
    monkeypatch.setattr(
        "ui.modules.project_management.shared.concurrency.QMessageBox.information",
        lambda _parent, _title, message: info_messages.append(message),
    )
    monkeypatch.setattr(
        "ui.modules.project_management.task.actions.QMessageBox.warning",
    ...
    assert info_messages == [
        "This task was updated by another user. The latest version has been reloaded. Review the changes and try again."
    ]


def test_project_edit_action_recovers_from_stale_write(monkeypatch):
    info_messages: list[str] = []
    warning_messages: list[str] = []
    ...
    monkeypatch.setattr(
        "ui.modules.project_management.project.actions.QMessageBox.warning",
        lambda _parent, _title, message: warning_messages.append(message),
    )
    probe = _Probe()
    ...
    assert warning_messages == []
    assert info_messages == [
        "This project was updated by another user. The latest version has been reloaded. Review the changes and try again."
    ]


def test_resource_edit_action_recovers_from_stale_write(monkeypatch):
    info_messages: list[str] = []
    warning_messages: list[str] = []

    class _ResourceService:
        def update_resource(self, **_kwargs):
            raise ConcurrencyError("stale")
    ...
    monkeypatch.setattr(
        "ui.modules.project_management.resource.actions.QMessageBox.warning",
        lambda _parent, _title, message: warning_messages.append(message),
    )
    probe = _Probe()
    ...
    assert warning_messages == []
    assert info_messages == [
        "This resource was updated by another user. The latest version has been reloaded. Review the changes and try again."
    ]


def test_cost_edit_action_recovers_from_stale_write(monkeypatch):
    info_messages: list[str] = []
    warning_messages: list[str] = []

    class _CostService:
        def update_cost_item(self, **_kwargs):
            raise ConcurrencyError("stale")
    ...
    class _Probe(CostActionsMixin):
        ...
        def reload_costs(self, preferred_cost_id=None):
            self.reloaded_with = preferred_cost_id
    ...
    monkeypatch.setattr(
        "ui.modules.project_management.cost.actions.QMessageBox.warning",
        lambda _parent, _title, message: warning_messages.append(message),
    )
    probe = _Probe()
    ...
    assert probe.reloaded_with == "cost-1"
    assert warning_messages == []
    assert info_messages == [
