...
- the preventive engine now also persists calendar-plan schedule instances, configurable horizon counts, and `fixed` versus `floating` schedule policy, with floating calendar plans recalculating future planned instances from actual completion instead of only from initial due dates
