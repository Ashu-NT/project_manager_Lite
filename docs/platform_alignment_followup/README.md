...
- maintenance is now cataloged as an available-but-disabled module by default, and the shell can expose the first `Maintenance Dashboard`, `Assets`, `Requests`, and `Reliability` workspaces when the module is licensed and enabled
