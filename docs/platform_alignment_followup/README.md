...
- maintenance now also exports recurring-failure and exception-review Excel workbooks through the shared report runtime, extending the earlier backlog, PM, downtime, and execution pack layer without creating a separate reporting stack
...
- the preventive engine now also persists calendar-plan schedule instances, configurable horizon counts, `fixed` versus `floating` schedule policy, configurable lead-time generation windows, and planner-facing forecast/regeneration tooling, with floating calendar plans recalculating future planned instances from actual completion instead of only from initial due dates
