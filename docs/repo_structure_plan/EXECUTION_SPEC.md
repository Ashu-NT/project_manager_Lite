...
- the collaboration workspace now uses a split `CollaborationWorkspace.qml` -> `CollaborationWorkspacePage.qml` pattern with local metrics and toolbar sections, matching the platform workspace layout style
- the projects workspace now uses a split `ProjectsWorkspace.qml` -> `ProjectsWorkspacePage.qml` pattern with page-owned KPI, toolbar filter popup, data table, detail, and dialog-host composition, matching the platform workspace layout style
- the resources workspace now uses a split `ResourcesWorkspace.qml` -> `ResourcesWorkspacePage.qml` pattern with local metrics, filters, catalog, detail, and dialog-host components, matching the platform workspace layout style
- the scheduling workspace now uses a split `SchedulingWorkspace.qml` -> `SchedulingWorkspacePage.qml` pattern with local toolbar, calendar, baseline, schedule, and metrics sections, matching the platform workspace layout style
- the financials workspace now uses a split `FinancialsWorkspace.qml` -> `FinancialsWorkspacePage.qml` pattern with local metrics, filters, catalog, detail, insight, and dialog-host components, matching the platform workspace layout style
- the tasks workspace now uses a split `TasksWorkspace.qml` -> `TasksWorkspacePage.qml` pattern with page-owned KPI, toolbar filter popup, bulk-actions, data table, detail, and dialog-host composition, matching the platform workspace layout style
- the risk and register workspaces now use real `RiskWorkspacePage.qml` / `RegisterWorkspacePage.qml` screens backed by shared register metrics, filters, catalog, detail, urgent-queue, and dialog-host components
...
- the remaining PM landing-zone-only workspaces are now `portfolio` and `timesheets`; `dashboard` is the first real read-only migrated slice, `projects`, `resources`, `risk`, `register`, and `tasks` are the first real CRUD migrated slices, `scheduling` is the first real PM operations slice, `financials` is the first real PM commercial-control slice, `collaboration` is the first real PM coordination slice, and active PM QWidget screens under `src/ui/*` remain active until each screen has a real QML replacement, presenter, view model, navigation rewrite, and tests
...
- `src/core/modules/project_management/api/desktop/collaboration.py` defines the PM collaboration desktop API contract, including notifications, mention inbox, recent activity, active presence, mark-read, and task-presence touch/clear mutations
...
- `src/core/modules/project_management/api/desktop/financials.py` defines the PM financials desktop API contract, including project/task/category selectors, cost-item CRUD, finance snapshot, ledger, cashflow, and analytics DTOs
- `src/core/modules/project_management/api/desktop/projects.py` defines the PM projects CRUD desktop API contract
...
- `src/ui_qml/modules/project_management/view_models/projects.py` defines PM projects overview, status, catalog, detail, and dialog state view models
...
- `src/ui_qml/modules/project_management/presenters/financials_workspace_presenter.py` maps the PM financials desktop API into typed project/task/category filters, cost-item catalog/detail, and finance insight view models
- `src/ui_qml/modules/project_management/presenters/projects_workspace_presenter.py` maps the PM projects desktop API into typed status, catalog, detail, and mutation-friendly view models
...
- `src/ui_qml/modules/project_management/presenters/tasks_workspace_presenter.py` maps the PM tasks desktop API into typed selector, catalog, detail, assignment/dependency execution, task-collaboration, and progress-friendly view models
...
- `src/ui_qml/modules/project_management/controllers/financials/financials_workspace_controller.py` exposes the financials workspace through a typed `ProjectManagementFinancialsWorkspaceController`
- `src/ui_qml/modules/project_management/controllers/projects/projects_workspace_controller.py` exposes the projects workspace through a typed `ProjectManagementProjectsWorkspaceController`
- `src/ui_qml/modules/project_management/controllers/register/register_workspace_controller.py` exposes both the risk and register workspaces through a typed `ProjectManagementRegisterWorkspaceController`
- `src/ui_qml/modules/project_management/controllers/resources/resources_workspace_controller.py` exposes the resources workspace through a typed `ProjectManagementResourcesWorkspaceController`
...
- `src/ui_qml/modules/project_management/qml_type_registration.py` plus `ProjectManagement/Controllers/typeinfo/*` now expose typed runtime/tooling registration for the PM catalog and dashboard/financials/projects/resources/risk/register/scheduling/tasks controllers
...
- `SchedulingWorkspacePage.qml` now renders API-backed project selection, working-calendar setup, holiday management, working-day calculation, schedule recalculation, baseline creation/deletion, baseline comparison, and critical-path sections through a typed controller and split QML sections
- `FinancialsWorkspacePage.qml` now renders API-backed project/category filters, cost-item catalog/detail, create/edit/delete dialogs, finance KPIs, cashflow, ledger, and analytics sections through a typed controller and split QML sections
- `ProjectsWorkspacePage.qml` now renders API-backed filters, catalog/detail sections, and create/edit/status/delete dialogs through a typed controller and split QML sections
- `RiskWorkspacePage.qml` and `RegisterWorkspacePage.qml` now render API-backed project/type/status/severity filters, catalog/detail sections, urgent review queues, and create/edit/delete dialogs through a typed controller and shared QML components
- `ResourcesWorkspacePage.qml` now renders API-backed active/category filters, catalog/detail sections, employee-linked worker setup, and create/edit/active-toggle/delete dialogs through a typed controller and split QML sections
- `TasksWorkspacePage.qml` now renders API-backed project/status filters, catalog/detail sections, and create/edit/progress/delete dialogs through a typed controller and split QML sections
- presenter/catalog scaffolding now calls the PM workspace desktop API for metadata, while the dashboard, financials, projects, resources, risk/register, scheduling, and tasks also consume screen-specific desktop API state; real workflow/query desktop APIs for the other workspaces remain pending
- the dashboard read-only snapshot API is the first PM screen-specific desktop API contract; `financials`, `projects`, `resources`, `risk/register`, `tasks`, and `scheduling` are the first PM mutation-capable QML contracts
- active PM QWidget screens remain active; no Widget screen has been deleted or replaced
- `tests/test_qml_project_management_presenters.py` covers route-to-presenter alignment, typed dashboard/financials/projects/resources/risk/register/scheduling/tasks controller exposure, named-module usage, QML-safe catalog maps, and guards against legacy Widget or infrastructure imports in the PM QML layer
- `tests/test_project_management_desktop_api.py` covers PM desktop workspace API descriptors, financials CRUD/snapshot, projects CRUD, register CRUD, resources CRUD, scheduling operations, tasks CRUD/progress contracts, and guards against QML or infrastructure imports from the desktop API layer
...
- the latest PM-focused verification batches pass with `14 passed`, `14 passed`, `7 passed`, `27 passed`, `2 passed`, `1 passed`, and `7 passed`; and a full `qmllint` scan across `src/ui_qml/**/*.qml` is clean
...
- `pytest tests/test_platform_admin_desktop_api.py tests/test_platform_control_desktop_api.py tests/test_platform_org_desktop_api.py tests/test_platform_runtime_desktop_api.py tests/test_document_admin_ui.py tests/test_phase_b_user_admin_ui.py tests/test_main_window_shell_navigation.py tests/test_qml_architecture_guardrails.py tests/test_architecture_guardrails.py -q`
  ...
  - `pytest src/tests/inventory_procurement/test_inventory_procurement_requisition.py src/tests/inventory_procurement/test_inventory_procurement_purchasing.py src/tests/inventory_procurement/test_inventory_import_export_reporting.py src/tests/inventory_procurement/test_inventory_procurement_ui.py -q`
  ...
  - `pytest tests/test_refactor_regressions.py tests/test_pro_set_v1_ui.py tests/test_dashboard_professional_panels.py src/tests/inventory_procurement/test_inventory_procurement_ui.py tests/test_main_window_shell_navigation.py tests/test_phase_b_user_admin_ui.py -q`
  - `pytest tests/test_architecture_guardrails.py::test_legacy_platform_control_ui_package_is_removed tests/test_architecture_guardrails.py::test_platform_control_workspace_package_exports_tabs tests/test_phase_b_user_admin_ui.py src/tests/inventory_procurement/test_inventory_procurement_purchasing.py tests/test_governance_tab_mode_toggle_ui.py -q`
  ...
  - `pytest tests/test_service_architecture.py tests/test_shared_collaboration_import_and_timesheets.py src/tests/inventory_procurement/test_inventory_import_export_reporting.py src/tests/inventory_procurement/test_inventory_procurement_foundation.py tests/test_project_management_platform_alignment.py tests/test_collaboration_import_timesheet_regressions.py -q`
...
- PM QML presenter/view-model scaffolding is complete for the route set, and `collaboration`, `dashboard`, `financials`, `portfolio`, `projects`, `resources`, `risk`, `register`, `scheduling`, `tasks`, and `timesheets` now have typed controllers plus split page sections backed by screen-specific desktop APIs; that UI progress remains valid and should not be reverted
- PM QML now binds to presenter-backed metadata through a typed `pmCatalog` and named PM controller/dialog/widget modules, with real screen-specific desktop APIs in place for every active PM route
- real PM workflow/query desktop APIs are now in place for `collaboration`, `dashboard`, `financials`, `portfolio`, `projects`, `resources`, `risk/register`, `scheduling`, `tasks`, and `timesheets`; continue deeper parity one migrated screen at a time
...
- PM Tasks QML now has API-backed project/status/priority/schedule filters, advanced task-query parsing, saved task views, catalog/detail sections, create/edit/progress/delete dialogs, bulk status/delete actions, bulk-status undo/redo, collaboration mention/notification/active-presence status metrics, assignment management, dependency management, assignment-period time-entry capture, task comments, mention-read flows, attachment/document-linked collaboration posting, controller-owned task-presence touch/clear flows for review/edit/progress/comment sessions, and active presence; deeper parity and final Widget deletion still remain until parity is completed
...
- added `src/core/modules/inventory_procurement/application/common/reference_service.py` and `src/core/modules/inventory_procurement/access/*` so inventory reference lookups and storeroom scope policy now live under the module-local `src` tree
...
- added module-local inventory APIs under `src/core/modules/inventory_procurement/api/desktop/{workspaces,dashboard}.py`, `api/desktop/catalog/{api,options,categories,items,models,serializers}.py`, `api/desktop/inventory/{api,options,storerooms,balances,movements,models,serializers}.py`, `api/desktop/reservations/{api,options,records,models,serializers}.py`, `api/desktop/procurement/{api,requisitions,purchase_orders,receipts,statuses}.py`, `api/desktop/pricing/{api,models}.py`, plus `api/desktop/__init__.py`; `api/http/__init__.py` stays only as the future transport placeholder
- wired the shared desktop registry in `src/api/desktop/runtime.py` to expose `inventory_procurement_{workspaces,catalog,inventory,reservations,procurement,dashboard,pricing}` on top of the transferred inventory services
- expanded the inventory foundation underneath that API with module-owned `storage_location`, `reorder_policy`, and `cycle_count` domain models plus matching repository contracts, persistence rows, migration coverage, `InventoryFoundationService`, entitlement-aware inventory foundation desktop snapshot assembly, and typed inventory presenter/controller/view-model state so the module can express warehouse hierarchy, replenishment policy, count governance, and cross-module availability before any further UI redesign
- kept the live inventory package branches under `src/core/modules/inventory_procurement/*` as `access/`, `application/{catalog,common,inventory,procurement}`, `contracts/{repositories,gateways}`, `domain/{catalog,inventory,procurement}`, `infrastructure/{exporters,importers,integrations,persistence,reporting}`, and `api/{desktop,http}`. There is still no separate `application/pricing/` or `domain/pricing/`; pricing stays an API/reporting projection
- inventory QML now has six real workspaces on top of that structure: `dashboard`, `catalog`, `inventory`, `reservations`, `procurement`, and `pricing`
- `src/ui_qml/modules/inventory_procurement/controllers/inventory/inventory_workspace_controller.py`, `presenters/inventory_workspace_presenter.py`, and `view_models/inventory.py` now provide the typed controller/presenter/view-model path for inventory stock operations, `controllers/reservations/reservations_workspace_controller.py`, `presenters/reservations_workspace_presenter.py`, and `view_models/reservations.py` provide the typed path for reservation search and execution flows, `controllers/procurement/procurement_workspace_controller.py`, `presenters/procurement_workspace_presenter.py`, and `view_models/procurement.py` provide the typed path for requisitions, purchase orders, and receipt posting, and `controllers/pricing/pricing_workspace_controller.py`, `presenters/pricing_workspace_presenter.py`, and `view_models/pricing.py` provide the typed path for stock/procurement reporting and supplier-pricing review
- `InventoryWorkspacePage.qml` now renders API-backed site/storeroom/item/movement filters, storeroom CRUD, stock balance review, movement history, and opening-balance/adjustment/issue/return/transfer dialogs through the typed inventory controller, `ReservationsWorkspacePage.qml` now renders reservation filters, catalog/detail state, and create/issue/release/cancel dialogs through the typed reservations controller, `ProcurementWorkspacePage.qml` now renders requisition filters and demand lines, purchase-order supplier commitments, and receipt-posting dialogs through the typed procurement controller, and `PricingWorkspacePage.qml` now renders inventory/pricing filters, KPI metrics, stock-status review, supplier-price lines, and report exports through the typed pricing controller
- regrouped the inventory-specific flat tests into `src/tests/inventory_procurement/*` and added `src/tests/conftest.py` as the shared fixture bridge back to the root `tests` helpers
...
- focused verification after the inventory desktop-API slice passes:
  ...
  - `conda run -n pmenv python -m pytest -q src/tests/inventory_procurement/test_inventory_procurement_desktop_api.py tests/test_service_architecture.py tests/test_architecture_guardrails.py tests/test_platform_runtime_desktop_api.py tests/test_platform_org_desktop_api.py tests/test_platform_admin_desktop_api.py tests/test_platform_control_desktop_api.py`
  - observed results after the inventory desktop-API transfer: `130 passed`
- focused verification after the inventory QML inventory, reservations, procurement, and pricing slices pass:
  - `python -m compileall -q src/core/modules/inventory_procurement src/ui_qml/modules/inventory_procurement tests`
  - full `pyside6-qmllint` across `src/ui_qml/**/*.qml` with the repo import roots
  - `conda run -n pmenv python -m pytest -q src/tests/inventory_procurement/test_inventory_procurement_desktop_api.py src/tests/inventory_procurement/test_qml_inventory_procurement_presenters.py src/tests/inventory_procurement/test_qml_inventory_procurement_routes.py`
  - `conda run -n pmenv python -m pytest -q tests/test_qml_shared_primitives.py tests/test_qml_architecture_guardrails.py`
  - `conda run -n pmenv python -m pytest -q tests/test_qml_migration_scaffold.py`
  - `conda run -n pmenv python -m pytest -q tests/test_qml_shell_migration.py`
  - `conda run -n pmenv python -m pytest -q tests/test_qml_offscreen_loading.py`
  - `conda run -n pmenv python -m pytest -q tests/test_main_window_shell_navigation.py`
  - observed results after the inventory QML inventory, reservations, procurement, and pricing slices: `23 passed` in the focused inventory desktop/QML/routes batch, and full `pyside6-qmllint` across `src/ui_qml/**/*.qml` is clean
- Slice 3 is now complete enough to advance to Slice 4: inventory backend code lives under `src/core/modules/inventory_procurement/*`, all planned inventory QML workspaces are real, inventory-specific tests are regrouped under `src/tests/inventory_procurement/*`, the old inventory QWidget UI remains only as temporary runtime fallback until the final QML cutover, and HTTP transport stays explicitly deferred to a later phase
