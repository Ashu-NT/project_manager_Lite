...
1. Split the remaining `core/platform/*` packages into `domain/`, `application/`, and `contracts/` without wrappers: `access`, `modules`, `org`, `party`, `approval`, `documents`, `notifications`, and `audit`.
