...
- platform controller types are now exposed to QML through the typed module `src/ui_qml/platform/qml/controllers/*`, with runtime registration in `src/ui_qml/platform/qml_type_registration.py`; workspace pages no longer declare controller objects as generic `QtObject`
...
- platform `admin` now also exposes controller-owned document focus, preview-state, linked-record, and document-structure workflows through `src/ui_qml/platform/presenters/document_management_presenter.py`, `src/ui_qml/platform/qml/widgets/DocumentDetailPanel.qml`, and focused document link/structure dialogs under `src/ui_qml/platform/qml/dialogs/*`
- platform `admin` now also exposes scoped access and account-security workflows through `src/ui_qml/platform/access_workspace_state.py`, `src/ui_qml/platform/presenters/access_workspace_presenter.py`, and `src/ui_qml/platform/qml/widgets/AccessSecurityPanel.qml`
...
- platform QML widgets now live under the named module `src/ui_qml/platform/qml/widgets/*`, including `OverviewSectionCard.qml`, `RecordListCard.qml`, `WorkspaceStateBanner.qml`, and `AdminCatalogPanel.qml`
- platform QML dialogs now live under the named module `src/ui_qml/platform/qml/dialogs/*`, including focused editor dialogs for organizations, sites, departments, employees, users, parties, documents, document links, and document structures plus `ApprovalDecisionDialog.qml` and `ModuleLifecycleDialog.qml`
...
- `src/ui_qml/platform/presenters/{runtime,admin_presenter,control_presenter,settings_presenter,control_queue_presenter,settings_catalog_presenter,organization_catalog_presenter,site_catalog_presenter,department_catalog_presenter,employee_catalog_presenter}.py`, `src/ui_qml/platform/view_models/{runtime,workspace}.py`, `src/ui_qml/platform/admin_workspace_state.py`, and `src/ui_qml/platform/qml/{widgets,dialogs}/*` now provide grouped overview, admin master-data actions, dialogs, action-list, and workspace-state surfaces for platform QML
...
- upgraded `src/ui_qml/platform/qml/workspaces/admin/AdminWorkspace.qml` from the first org/site/department/employee surface into a broader admin QML surface for organizations, sites, departments, employees, users, parties, and documents, backed by `src/ui_qml/platform/qml/widgets/AdminCatalogPanel.qml` and `src/ui_qml/platform/qml/dialogs/{OrganizationEditorDialog,SiteEditorDialog,DepartmentEditorDialog,EmployeeEditorDialog,UserEditorDialog,PartyEditorDialog,DocumentEditorDialog}.qml`
- added `src/ui_qml/platform/access_workspace_state.py`, `src/ui_qml/platform/presenters/access_workspace_presenter.py`, and `src/ui_qml/platform/qml/widgets/AccessSecurityPanel.qml` so `platform.admin` now owns scoped access grants and account-security actions through a separate controller instead of growing `admin_workspace_state.py` into another dump file
...
- added `src/ui_qml/platform/qml/widgets/DocumentDetailPanel.qml` plus `src/ui_qml/platform/qml/dialogs/{DocumentLinkEditorDialog,DocumentStructureEditorDialog}.qml` so the document admin slice no longer depends on the legacy widget document dialogs for those workflows
