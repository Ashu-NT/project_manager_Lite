...
- project-management projects QML now renders the first mutable PM workspace slice through `ProjectsWorkspacePage.qml`, `ProjectsDetailSection.qml`, `ProjectsDialogHost.qml`, `ProjectManagement.Dialogs/{ProjectEditorDialog,ProjectStatusDialog}.qml`, and shared `App.Widgets/{KpiStrip,TableToolbar,DataTable,SectionDetailPage}` primitives
- project-management tasks now has a workflow desktop API contract under `src/core/modules/project_management/api/desktop/tasks.py`, with typed project/status selectors plus task create, update, progress, assignment, dependency, and task-collaboration flows mapped onto the refactored `application/tasks` service
- project-management tasks QML now renders the next mutable PM workspace slice through `TasksWorkspacePage.qml`, `TasksDetailPanel.qml`, `TasksAssignmentsSection.qml`, `TasksDependenciesSection.qml`, `TasksTimeEntriesSection.qml`, `TasksCollaborationSection.qml`, `TasksBulkActionsSection.qml`, `TasksDialogHost.qml`, `ProjectManagement.Dialogs/{TaskEditorDialog,TaskProgressDialog,TaskAssignmentEditorDialog,TaskAssignmentHoursDialog,TaskDependencyEditorDialog,TaskCollaborationComposerDialog}.qml`, and shared `App.Widgets/{KpiStrip,TableToolbar,DataTable,SectionDetailPage}` plus `ProjectManagement.Widgets/TimesheetEntriesCard.qml`, with advanced task-query filters, saved task views persisted through the shared `QSettings` key, controller-owned task-presence sessions for review/edit/progress/comment flows, and detailed assignment-period labor capture reusing the PM timesheets desktop API
- project-management scheduling now has an operations desktop API contract under `src/core/modules/project_management/api/desktop/scheduling.py`, with typed project selectors, working-calendar setup, holiday management, working-day calculation, schedule recalculation, baseline creation/deletion, and baseline comparison mapped onto the refactored scheduling/reporting services
- project-management scheduling QML now renders the next real PM operations slice through `SchedulingWorkspacePage.qml`, `SchedulingToolbarSection.qml`, `SchedulingCalendarSection.qml`, `SchedulingBaselineSection.qml`, `SchedulingScheduleSection.qml`, `SchedulingMetricsSection.qml`, and `ProjectManagement.Widgets/RecordListCard.qml`
- project-management financials now has an operations desktop API contract under `src/core/modules/project_management/api/desktop/financials.py`, with typed project/task/category selectors plus cost-item create, update, delete, ledger, cashflow, and finance analytics contracts mapped onto the refactored `application/financials` services
- project-management financials QML now renders the next real PM commercial-control slice through `FinancialsWorkspacePage.qml`, `FinancialsMetricsSection.qml`, `FinancialsFiltersSection.qml`, `FinancialsCatalogSection.qml`, `FinancialsDetailSection.qml`, `FinancialsInsightsSection.qml`, `FinancialsDialogHost.qml`, `ProjectManagement.Dialogs/CostItemEditorDialog.qml`, and `ProjectManagement.Widgets/RecordListCard.qml`
- project-management risk/register now has a shared desktop API contract under `src/core/modules/project_management/api/desktop/register.py`, with typed project/type/status/severity selectors plus register-entry create, update, and delete commands mapped onto the refactored `application/risk/register_service.py`
- project-management risk/register QML now renders the next real PM governance slice through `RiskWorkspacePage.qml`, `RegisterWorkspacePage.qml`, `ProjectManagement.Dialogs/RegisterEntryEditorDialog.qml`, and shared `ProjectManagement.Widgets/Register{Metrics,Filters,Catalog,Detail,Urgent,DialogHost}*.qml` components
- project-management resources now has a CRUD desktop API contract under `src/core/modules/project_management/api/desktop/resources.py`, with typed worker-type/category/employee selectors plus create, update, active-toggle, and delete commands mapped onto the refactored `application/resources` service
- project-management resources QML now renders the next mutable PM workspace slice through `ResourcesWorkspacePage.qml`, `ResourcesMetricsSection.qml`, `ResourcesFiltersSection.qml`, `ResourcesCatalogSection.qml`, `ResourcesDetailSection.qml`, `ResourcesDialogHost.qml`, `ProjectManagement.Dialogs/ResourceEditorDialog.qml`, and `ProjectManagement.Widgets/RecordListCard.qml`
- project-management collaboration now has a workflow desktop API contract under `src/core/modules/project_management/api/desktop/collaboration.py`, with notifications, mention inbox, recent activity, active presence, mark-read, and task-presence touch/clear commands mapped onto the refactored `application/tasks/collaboration_service.py`
- project-management collaboration QML now renders the next real PM coordination slice through `CollaborationWorkspacePage.qml`, `CollaborationMetricsSection.qml`, `CollaborationToolbarSection.qml`, and `ProjectManagement.Widgets/RecordListCard.qml`
- project-management portfolio now has a planning desktop API contract under `src/core/modules/project_management/api/desktop/portfolio.py`, with intake-status/dependency selectors plus scoring-template, intake, scenario, dependency, heatmap, executive-action, and scenario-evaluation/comparison contracts mapped onto the refactored `application/projects/portfolio_service.py`
...
- project-management QML now also exposes typed `ProjectManagement.Controllers` / `ProjectManagement.Widgets` modules, split `collaboration/dashboard/financials/portfolio/projects/resources/risk/register/scheduling/tasks/timesheets` page/controller slices, and no active PM route now falls back to the old placeholder-page pattern
...
- the latest PM-focused verification batches pass with `18 passed`, `18 passed`, `32 passed`, `10 passed`, `7 passed`, `2 passed`, `1 passed`, and `7 passed`; `python -m compileall -q src/core/modules/project_management src\\api\\desktop src\\ui_qml tests` passes; and a full `qmllint` scan across `src/ui_qml/**/*.qml` is clean
...
- maintenance ORM rows currently live under `src/infra/persistence/orm/maintenance/*`; that folder remains a transitional Slice 1 de-mixing home until the maintenance slice moves it under module-owned infrastructure
- platform repositories and mappers now use the same structure as module persistence: `src/core/platform/infrastructure/persistence/repositories/<area>.py`, `mappers/<area>.py`, and `orm/<area>.py`; remaining business-module repositories still use transitional module infrastructure only until their owning slice moves them under module-owned infrastructure
...
- `src/ui_qml/modules/project_management/context.py` exposes a QML-safe PM workspace catalog used by the active PM QML workspaces
...
2. The repo should use global `src/infra/persistence/orm/` only for shared SQLAlchemy metadata concerns, and module-local `infrastructure/persistence/` for module-owned persistence.
   ...
   - existing global module ORM packages such as `src/infra/persistence/orm/maintenance/*` are temporary and must be removed during the owning module slice after rows move to `src/core/modules/<module>/infrastructure/persistence/orm/`
...
- moved platform ORM model files out of `infra/platform/db/`; business-module ORM rows landed in temporary global de-mixing homes until their module slices move them under module-local infrastructure:
  ...
  - `inventory_models.py` to `src/core/modules/inventory_procurement/infrastructure/persistence/orm/{catalog,inventory,procurement}.py`
...
- rewired inventory persistence adapters to the split module-local ORM files under `src.core.modules.inventory_procurement.infrastructure.persistence.orm.{catalog,inventory,procurement}` instead of importing inventory rows through `src.infra.persistence.orm.platform.models`
...
- in `conda run -n pmenv`, direct import of `src.infra.persistence.orm.Base`, PM split ORM files for project/task/resource/baseline/collaboration/portfolio, the split inventory ORM files under `src.core.modules.inventory_procurement.infrastructure.persistence.orm.{catalog,inventory,procurement}`, and platform ORM rows such as `TimeEntryORM`, `TimesheetPeriodORM`, `UserORM`, and `OrganizationORM` passes
...
- module-owned maintenance ORM rows under `src/infra/persistence/orm/maintenance` remain transitional and must move to that module's `infrastructure/persistence/orm/` package during its slice
...
- PM QML presenter/view-model scaffolding is in place for every PM workspace route, and `collaboration`, `dashboard`, `financials`, `portfolio`, `projects`, `resources`, `risk`, `register`, `scheduling`, `tasks`, and `timesheets` now have typed controllers and split page sections backed by screen-specific desktop APIs; that UI progress remains valid and should not be reverted
- PM QML now consumes presenter-backed metadata through a typed QML catalog/controller surface plus named `ProjectManagement.Controllers`, `ProjectManagement.Dialogs`, and `ProjectManagement.Widgets` modules, with real screen-specific desktop APIs in place for every active PM route
...
- PM Tasks QML now renders API-backed project/status/priority/schedule filters, advanced task-query parsing, saved task views, catalog/detail panels, create/edit/progress/delete dialogs, bulk status/delete actions, bulk-status undo/redo, collaboration mention/notification/active-presence status metrics, assignment management, dependency management, assignment-period time-entry capture, task comments, mention-read flows, attachment/document-linked collaboration posting, controller-owned task-presence touch/clear flows for review/edit/progress/comment sessions, and active presence through a typed controller and split section layout; deeper parity and final QWidget deletion still remain until parity is completed
...
| none | `src/core/modules/inventory_procurement/api/desktop/*.py` and reserved `api/http/*.py` | create module-local desktop adapters now and keep HTTP transport for a later phase |
...
- kept the live inventory package branches under `src/core/modules/inventory_procurement/*` as `access/`, `application/{catalog,common,inventory,procurement}`, `contracts/{repositories,gateways}`, `domain/{catalog,inventory,procurement}`, `infrastructure/{exporters,importers,integrations,persistence,reporting}`, and `api/{desktop,http}`. There is still no separate `application/pricing/` or `domain/pricing/`; pricing stays an API/reporting projection over the transferred inventory and procurement services
- inventory QML now has real typed workspaces for `dashboard`, `catalog`, `inventory`, `reservations`, `procurement`, and `pricing`
- kept the inventory QML structure aligned with platform and PM: `src/ui_qml/modules/inventory_procurement/controllers/inventory/inventory_workspace_controller.py`, `presenters/inventory_workspace_presenter.py`, `view_models/inventory.py`, `qml/workspaces/inventory/{InventoryWorkspacePage,InventoryDialogHost,InventoryFiltersSection,InventoryMetricsSection,StoreroomCatalogSection,StoreroomDetailSection,BalanceCatalogSection,BalanceDetailSection,TransactionsSection}.qml`, `controllers/reservations/reservations_workspace_controller.py`, `presenters/reservations_workspace_presenter.py`, `view_models/reservations.py`, `qml/workspaces/reservations/{ReservationsWorkspacePage,ReservationsDialogHost,ReservationsFiltersSection,ReservationsMetricsSection,ReservationsCatalogSection,ReservationDetailSection}.qml`, `controllers/procurement/procurement_workspace_controller.py`, `presenters/procurement_workspace_presenter.py`, `view_models/procurement.py`, `qml/workspaces/procurement/{ProcurementWorkspacePage,ProcurementDialogHost,ProcurementFiltersSection,ProcurementMetricsSection,RequisitionCatalogSection,RequisitionDetailSection,RequisitionLinesSection,PurchaseOrderCatalogSection,PurchaseOrderDetailSection,PurchaseOrderLinesSection,ReceiptHistorySection}.qml`, `controllers/pricing/pricing_workspace_controller.py`, `presenters/pricing_workspace_presenter.py`, `view_models/pricing.py`, `qml/workspaces/pricing/{PricingWorkspace,PricingWorkspacePage,PricingMetricsSection,PricingFiltersSection,PricingExportsSection,PricingStockSection,PricingSupplierPricingSection}.qml`, and `qml/InventoryProcurement/Dialogs/{StoreroomEditorDialog,StockMovementDialog,StockTransferDialog,ReservationCreateDialog,ReservationIssueDialog,RequisitionEditorDialog,RequisitionLineDialog,PurchaseOrderEditorDialog,PurchaseOrderLineDialog,ReceiptPostDialog}.qml`
...
- focused verification after the inventory QML inventory, reservations, procurement, and pricing slices pass:
  ...
  - observed results after the inventory QML inventory, reservations, procurement, and pricing slices: `23 passed` in the focused inventory desktop/QML/routes batch, full `pyside6-qmllint` across `src/ui_qml/**/*.qml` is clean, and the shared shell/guardrail Qt files continue to pass in their focused batches
...
- after the completed asset, preventive, reliability, work-request, work-order, document, downtime-event, domain/contracts, persistence/reporting, workspace desktop-API, assets desktop-API, dashboard desktop-API, planner desktop-API, reliability desktop-API, work-requests desktop-API, and work-orders desktop-API transfers, maintenance QML now has dashboard/assets/planner/preventive/reliability/work_requests/work_orders active, and Slice 4 is complete enough to advance to Slice 5 while legacy QWidget maintenance screens stay in place until the final cutover
