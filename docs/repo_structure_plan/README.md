...
- added the first real inventory QML workspaces under `src/ui_qml/modules/inventory_procurement/*`: `dashboard` stays live, `catalog` now covers category/item CRUD plus document linking, and `inventory` now covers storeroom CRUD, stock balances, recent movements, and opening-balance/adjustment/issue/return/transfer flows through the typed `InventoryProcurementInventoryWorkspaceController`
- kept the inventory QML structure aligned with platform and PM: `src/ui_qml/modules/inventory_procurement/controllers/inventory/inventory_workspace_controller.py`, `presenters/inventory_workspace_presenter.py`, `view_models/inventory.py`, `qml/workspaces/inventory/{InventoryWorkspacePage,InventoryDialogHost,InventoryFiltersSection,InventoryMetricsSection,StoreroomCatalogSection,StoreroomDetailSection,BalanceCatalogSection,BalanceDetailSection,TransactionsSection}.qml`, and `qml/InventoryProcurement/Dialogs/{StoreroomEditorDialog,StockMovementDialog,StockTransferDialog}.qml`
...
- focused verification after the inventory QML inventory-operations slice passes:
  ...
  - observed results after the inventory QML inventory-operations slice: `16 passed`, `40 passed`, `5 passed`, `7 passed`, `1 passed`, and `7 passed` in the focused batches; the grouped conda/Qt batch still shows the existing environment quirk, so those files were verified individually
- remaining Slice 3 work stays exactly on the plan: keep the module-local inventory desktop APIs stable, add real inventory HTTP routers on top of `src/core/modules/inventory_procurement/api/http/*`, migrate the remaining inventory QML workspaces (`reservations`, `procurement`, and later `pricing`), regroup tests, and keep the old inventory QWidget UI only as temporary runtime fallback until the final QML cutover
