...
- focused verification after the inventory QML inventory, reservations, and procurement slices pass:
  ...
  - observed results after the inventory QML inventory, reservations, and procurement slices: `20 passed`, `40 passed`, `5 passed`, `7 passed`, `1 passed`, and `7 passed` in the focused batches; the grouped conda/Qt batch still shows the existing environment quirk, so those files were verified individually
- remaining Slice 3 work stays exactly on the plan: keep the module-local inventory desktop APIs stable, add real inventory HTTP routers on top of `src/core/modules/inventory_procurement/api/http/*`, migrate the remaining inventory QML workspace (`pricing`), regroup tests, and keep the old inventory QWidget UI only as temporary runtime fallback until the final QML cutover
