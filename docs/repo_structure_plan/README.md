...
- PM QML landing-zone routes are now in place for every Slice 2 PM workspace, and `dashboard`, `projects`, `resources`, `tasks`, plus `scheduling` have moved beyond placeholders into real typed workspaces; keep that progress, but do not let it replace the remaining structure transfer work
