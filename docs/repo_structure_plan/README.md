...
- project-management tasks now has a workflow desktop API package under `src/core/modules/project_management/api/desktop/tasks/`, with typed project/status selectors plus task create, update, progress, assignment, dependency, and task-collaboration flows mapped onto the refactored `application/tasks` service
