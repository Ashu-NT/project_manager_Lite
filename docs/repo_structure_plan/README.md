...
- project-management QML now also exposes typed `ProjectManagement.Controllers` / `ProjectManagement.Widgets` modules, split `dashboard/financials/projects/resources/tasks/scheduling` page/controller slices, and a shared placeholder-page pattern for the still-unmigrated PM workspaces aligned with the platform QML structure
...
- the latest PM-focused verification batches pass with `14 passed`, `14 passed`, `7 passed`, `27 passed`, `2 passed`, `1 passed`, and `7 passed`; `python -m compileall -q src/core/modules/project_management src\\api src\\ui_qml tests` passes; and a full `qmllint` scan across `src/ui_qml/**/*.qml` is clean
...
- PM QML landing-zone routes are now in place for every Slice 2 PM workspace, and `dashboard`, `financials`, `projects`, `resources`, `tasks`, plus `scheduling` have moved beyond placeholders into real typed workspaces; keep that progress, but do not let it replace the remaining structure transfer work
- PM QML presenter/view-model scaffolding is in place for every PM workspace route, and `dashboard`, `projects`, `resources`, `tasks`, plus `scheduling` now have typed controllers and split page sections backed by screen-specific desktop APIs; that UI progress remains valid and should not be reverted
- PM QML now consumes presenter-backed metadata through a typed QML catalog/controller surface plus named `ProjectManagement.Controllers`, `ProjectManagement.Dialogs`, and `ProjectManagement.Widgets` modules; outside `dashboard`, `projects`, `resources`, `tasks`, and `scheduling`, workflow/query API wiring still needs to come from the refactored module structure first
