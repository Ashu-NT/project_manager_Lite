...
- PM Tasks QML now renders API-backed project/status filters, catalog/detail panels, create/edit/progress/delete dialogs, assignment management, and dependency management through a typed controller and split section layout; task-level collaboration side panels, bulk actions, richer time-entry workflows, and deeper parity remain on the QWidget task workspace until parity is completed
