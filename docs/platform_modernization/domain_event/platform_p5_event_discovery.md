...
| `ModuleLicensed`/`ModuleLicenseRevoked`/`ModuleEnabled`/`ModuleDisabled`/`ModuleLifecycleTransitioned` | `OrganizationScope` | module entitlement collection (`module_entitlements` scope code) -- **resolved (P5B-3): only settings workspace genuinely reads it; control and access workspaces were traced end-to-end and read no module-entitlement state at all (see the P5B-3 report's corrected consumer classification, including the access-workspace storeroom-scope-type false lead from the fake test helper's crafted data)** | `modules_changed` (retired completely) | `settings_workspace_controller.py:126` only | module-catalog mutation mixin | `application/tenant/modules/event_handlers/view_invalidation.py` (implemented) | `ModuleEntitlementViewInvalidationAdapter` (implemented, P5B-3) | done -- `modules_changed` deleted (Signal + bridge-spec entry) |
