...
### Current UI Coverage Vs Pending Authoring Coverage

The current implementation already covers the first operational workbenches:
...
Those screens are intentionally runtime-first:
...
They are not yet the full maintenance setup surface.

Still pending as explicit maintenance authoring/setup UI:
...
- guided `Work Order` authoring/planning forms beyond queue/detail execution

This is an important distinction for the blueprint:

- current UI = operational runtime
- pending UI = setup, library, and authoring surfaces

That setup backlog should stay explicit while the maintenance shell grows out the rest of its dedicated `Libraries` / `Setup` area.
...
- locations
- systems
- asset libraries and hierarchy definitions
- task templates
- task step templates
- trigger logic
- default assignees
...
- sensor registry definitions and source mappings
- request-type defaults and work-order planning defaults
...
Recommended maintenance shell groups once the setup side is built:
...
The current implementation now covers the first `Libraries` slice as well.
The remaining gap is the rest of the authoring/setup surface under `Libraries`.

Recommended future `Libraries` workspaces:
...
- the runtime tabs are now connected, and `Requests` now has first guided intake plus conversion actions
- preventive requests now carry their source plan and selected task package into converted work orders
- `Work Orders` and `Planner` are still primarily queue/workbench surfaces
- richer work-order authoring, richer planner-side scheduling actions, and deeper dispatch/release controls are still later slices
...
- done: lead-time generation windows now allow calendar plans to enter due state and generate work ahead of the scheduled due date
- done: planner-facing forecast preview and explicit horizon regeneration tooling now expose schedule windows and persisted instance state in the preventive runtime
- next: blackout calendars and packaging/grouping
...
- next preventive-engine hardening:
  ...
  - lead-time generation windows. Done.
...
- recurring-failure and exception-review workbook exports on top of the reliability and sensor-exception runtime. Started.
...
- preventive engine hardening still pending:
  ...
  - generation forecast / preview service. Started.
