...
class BaselineViewInvalidationAdapter(QObject):
    projectBaselineStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != BASELINE_CATEGORY:
            return
        if hint.scope_code == BASELINE_PROJECT_SCOPE_CODE:
            self.projectBaselineStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["BaselineViewInvalidationAdapter"]
