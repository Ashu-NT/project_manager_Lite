...
class ProjectViewInvalidationAdapter(QObject):
    projectListStale = Signal(str)  # project_id (payload passthrough, org-wide fact)
    projectDetailStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != PROJECT_CATEGORY:
            return
        if hint.scope_code == PROJECT_LIST_SCOPE_CODE:
            self.projectListStale.emit(hint.entity_id or "")
        elif hint.scope_code == PROJECT_DETAIL_SCOPE_CODE:
            self.projectDetailStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["ProjectViewInvalidationAdapter"]
