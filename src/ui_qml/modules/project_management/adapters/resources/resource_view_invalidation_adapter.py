...
class ResourceViewInvalidationAdapter(QObject):
    resourceListStale = Signal(str)  # resource_id
    resourceCapabilitiesStale = Signal(str)  # resource_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != RESOURCE_CATEGORY:
            return
        if hint.scope_code == RESOURCE_LIST_SCOPE_CODE:
            self.resourceListStale.emit(hint.entity_id or "")
        elif hint.scope_code == RESOURCE_CAPABILITIES_SCOPE_CODE:
            self.resourceCapabilitiesStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["ResourceViewInvalidationAdapter"]
