...
class TaskCommentViewInvalidationAdapter(QObject):
    taskCommentsStale = Signal(str)  # task_id
    collaborationWorkspaceStale = Signal(str)  # task_id (payload passthrough, org-wide fact)
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != TASK_COMMENT_CATEGORY:
            return
        if hint.scope_code == TASK_COMMENT_SCOPE_CODE:
            self.taskCommentsStale.emit(hint.entity_id or "")
        elif hint.scope_code == COLLABORATION_WORKSPACE_SCOPE_CODE:
            self.collaborationWorkspaceStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["TaskCommentViewInvalidationAdapter"]
