...
class PortfolioViewInvalidationAdapter(QObject):
    portfolioWorkspaceStale = Signal(str)  # organization_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != PORTFOLIO_CATEGORY:
            return
        if hint.scope_code == PORTFOLIO_WORKSPACE_SCOPE_CODE:
            self.portfolioWorkspaceStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["PortfolioViewInvalidationAdapter"]
