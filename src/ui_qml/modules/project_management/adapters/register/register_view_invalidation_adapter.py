...
class RegisterViewInvalidationAdapter(QObject):
    registerWorkspaceStale = Signal(str)  # project_id
    registerProjectStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != REGISTER_CATEGORY:
            return
        if hint.scope_code == REGISTER_WORKSPACE_SCOPE_CODE:
            self.registerWorkspaceStale.emit(hint.entity_id or "")
        elif hint.scope_code == REGISTER_PROJECT_SCOPE_CODE:
            self.registerProjectStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["RegisterViewInvalidationAdapter"]
