...
class BudgetViewInvalidationAdapter(QObject):
    budgetPlanningStale = Signal(str)  # project_id
    budgetProjectSummaryStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != BUDGET_CATEGORY:
            return
        if hint.scope_code == BUDGET_PLANNING_SCOPE_CODE:
            self.budgetPlanningStale.emit(hint.entity_id or "")
        elif hint.scope_code == BUDGET_PROJECT_SUMMARY_SCOPE_CODE:
            self.budgetProjectSummaryStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["BudgetViewInvalidationAdapter"]
