...
class ForecastViewInvalidationAdapter(QObject):
    forecastPlanningStale = Signal(str)  # project_id
    forecastApprovedBasisStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != FORECAST_CATEGORY:
            return
        if hint.scope_code == FORECAST_PLANNING_SCOPE_CODE:
            self.forecastPlanningStale.emit(hint.entity_id or "")
        elif hint.scope_code == FORECAST_APPROVED_BASIS_SCOPE_CODE:
            self.forecastApprovedBasisStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["ForecastViewInvalidationAdapter"]
