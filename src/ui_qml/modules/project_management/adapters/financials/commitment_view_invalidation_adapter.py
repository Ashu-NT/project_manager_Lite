...
class CommitmentViewInvalidationAdapter(QObject):
    commitmentListStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != COMMITMENT_CATEGORY:
            return
        if hint.scope_code == COMMITMENT_LIST_SCOPE_CODE:
            self.commitmentListStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["CommitmentViewInvalidationAdapter"]
