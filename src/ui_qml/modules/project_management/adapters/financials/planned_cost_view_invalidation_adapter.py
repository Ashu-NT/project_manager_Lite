...
class PlannedCostViewInvalidationAdapter(QObject):
    plannedCostSnapshotStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != PLANNED_COST_CATEGORY:
            return
        if hint.scope_code == PLANNED_COST_SNAPSHOT_SCOPE_CODE:
            self.plannedCostSnapshotStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["PlannedCostViewInvalidationAdapter"]
