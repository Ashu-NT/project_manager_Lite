...
class PurchaseOrderViewInvalidationAdapter(QObject):
    purchaseOrderListStale = Signal(str)  # purchase_order_id
    purchaseOrderDetailStale = Signal(str)  # purchase_order_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != PROCUREMENT_CATEGORY:
            return
        if hint.scope_code == PURCHASE_ORDER_LIST_SCOPE_CODE:
            self.purchaseOrderListStale.emit(hint.entity_id or "")
        elif hint.scope_code == PURCHASE_ORDER_DETAIL_SCOPE_CODE:
            self.purchaseOrderDetailStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["PurchaseOrderViewInvalidationAdapter"]
