...
class InventoryCatalogViewInvalidationAdapter(QObject):
    itemListStale = Signal(str)  # item_id
    itemCategoryListStale = Signal(str)  # category_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != INVENTORY_CATALOG_CATEGORY:
            return
        if hint.scope_code == ITEM_LIST_SCOPE_CODE:
            self.itemListStale.emit(hint.entity_id or "")
        elif hint.scope_code == ITEM_CATEGORY_LIST_SCOPE_CODE:
            self.itemCategoryListStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["InventoryCatalogViewInvalidationAdapter"]
