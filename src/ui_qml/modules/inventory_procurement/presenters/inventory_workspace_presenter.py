...
class InventoryInventoryWorkspacePresenter:
    ...
    def build_workspace_state(
        ...
        *,
        ...
        return InventoryInventoryWorkspaceViewModel(
            ...
            foundation=self._build_foundation(
    ...
    def _build_foundation(
        ...
        *,
        ...
        snapshot = self._desktop_api.build_foundation_snapshot(
        ...
        return InventoryInventoryFoundationViewModel(
            ...
            location_type_options=tuple(
                InventorySelectorOptionViewModel(value=option.value, label=option.label)
                for option in self._desktop_api.list_location_types()
            ),
            cycle_count_status_options=tuple(
                InventorySelectorOptionViewModel(value=option.value, label=option.label)
                for option in self._desktop_api.list_cycle_count_statuses()
            ),
            locations=tuple(
                InventoryRecordViewModel(
                    id=row.id,
            ...
            valuation_signals=tuple(
                self._to_foundation_signal_record(row) for row in snapshot.valuation_signals
            ),
            tracking_signals=tuple(
                self._to_foundation_signal_record(row) for row in snapshot.tracking_signals
            ),
            activity_signals=tuple(
                self._to_foundation_signal_record(row) for row in snapshot.activity_signals
            ),
        )

    @staticmethod
    def _to_foundation_signal_record(row) -> InventoryRecordViewModel:
        return InventoryRecordViewModel(
            id=row.id,
            title=row.title,
            status_label=row.status_label,
            ...
            can_secondary_action=False,
            can_tertiary_action=False,
            state={"signalId": row.id, "tone": row.tone},
        )
