...
// Site Detail's Departments tab: a real, site_id-scoped, server-paginated
// management view -- the shared platform Department master filtered to
// this site, using the same canonical AdminEntityWorkspace stack
// Organization Detail's own Departments tab uses (see
// OrganizationDepartmentsSection.qml). Row activation opens the standalone
// Departments workspace's own Department Detail page rather than nesting a
// second copy of that UI here. All state (catalog/page/search/filter/
// selection) is owned by the orchestrator (AdminSiteDetailPage.qml); this
...
Item {
    ...
    PlatformComponents.AdminEntityWorkspace {
        ...
        tableId: "site.detail.departments.table"
