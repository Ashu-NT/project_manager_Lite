...
class CostEntryViewInvalidationAdapter(QObject):
    costEntryListStale = Signal(str)  # project_id
    costEntryActualsStale = Signal(str)  # project_id
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category != COST_ENTRY_CATEGORY:
            return
        if hint.scope_code == COST_ENTRY_LIST_SCOPE_CODE:
            self.costEntryListStale.emit(hint.entity_id or "")
        elif hint.scope_code == COST_ENTRY_ACTUALS_SCOPE_CODE:
            self.costEntryActualsStale.emit(hint.entity_id or "")

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["CostEntryViewInvalidationAdapter"]
