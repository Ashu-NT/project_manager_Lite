...
class DepartmentViewInvalidationAdapter(QObject):
    departmentCollectionStale = Signal()
    ...
    def _on_hint(self, hint: ViewInvalidationHint) -> None:
        if hint.category == DEPARTMENT_CATEGORY and hint.scope_code == DEPARTMENT_LIST_SCOPE_CODE:
            self.departmentCollectionStale.emit()

    def dispose(self) -> None:
        self._subscription.dispose()


__all__ = ["DepartmentViewInvalidationAdapter"]
