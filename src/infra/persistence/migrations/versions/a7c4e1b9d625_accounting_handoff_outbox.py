...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            [
                "project_accounting_handoffs.tenant_id",
                "project_accounting_handoffs.organization_id",
                "project_accounting_handoffs.project_id",
                "project_accounting_handoffs.id",
            ],
            ondelete="RESTRICT",
        ),
        sa.UniqueConstraint(
    ...
    install_immutable_row_guards(
        op, op.get_bind().dialect.name, ("project_accounting_handoffs",)
    )
    install_envelope_guards(
        op,
        op.get_bind().dialect.name,
        ("project_accounting_outbox",),
        extra_columns=("project_id",),
    )
...
def downgrade():
    ...
    if op.get_bind().dialect.name == "postgresql":
        op.execute(
            "DROP FUNCTION IF EXISTS prevent_project_accounting_handoffs_mutation()"
        )
        op.execute(
            "DROP FUNCTION IF EXISTS protect_project_accounting_outbox_envelope()"
        )
