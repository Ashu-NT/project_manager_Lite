...
def build_maintenance_service_bundle(
    ...
    location_repo = SqlAlchemyMaintenanceLocationRepository(platform_services.session, tenant_id_provider=_tid)
    system_repo = SqlAlchemyMaintenanceSystemRepository(platform_services.session, tenant_id_provider=_tid)
    asset_repo = SqlAlchemyMaintenanceAssetRepository(platform_services.session, tenant_id_provider=_tid)
    ...
    preventive_plan_repo = SqlAlchemyMaintenancePreventivePlanRepository(platform_services.session, tenant_id_provider=_tid)
    ...
    sensor_repo = SqlAlchemyMaintenanceSensorRepository(platform_services.session, tenant_id_provider=_tid)
    ...
    work_request_repo = SqlAlchemyMaintenanceWorkRequestRepository(platform_services.session, tenant_id_provider=_tid)
    work_order_repo = SqlAlchemyMaintenanceWorkOrderRepository(platform_services.session, tenant_id_provider=_tid)
    ...
    document_repo = SqlAlchemyDocumentRepository(platform_services.session, tenant_id_provider=_tid)
    document_link_repo = SqlAlchemyDocumentLinkRepository(platform_services.session)
    document_structure_repo = SqlAlchemyDocumentStructureRepository(platform_services.session, tenant_id_provider=_tid)
    user_repo = SqlAlchemyUserRepository(platform_services.session)
    employee_repo = SqlAlchemyEmployeeRepository(platform_services.session, tenant_id_provider=_tid)
    time_entry_repo = SqlAlchemyTimeEntryRepository(platform_services.session, tenant_id_provider=_tid)
    timesheet_period_repo = SqlAlchemyTimesheetPeriodRepository(platform_services.session, tenant_id_provider=_tid)
