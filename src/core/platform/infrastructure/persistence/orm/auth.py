...
class AuthorizationMigrationBatchORM(Base):
...
Index(
    ...
    AuthorizationMigrationBatchORM.status,
)


class LegacyRoleBindingMigrationRecordORM(Base):
    # RBAC-TRANSITION-ONLY: Drop after CANONICAL_ONLY and evidence retention.
    __tablename__ = "legacy_role_binding_migration_records"
    __table_args__ = (
        UniqueConstraint(
            "batch_id",
            "legacy_binding_id",
    ...
    # Source IDs intentionally have no foreign keys: the immutable snapshot must
    # survive retirement of the legacy row.
    legacy_binding_id: Mapped[str] = mapped_column(String, nullable=False)
    source_user_id: Mapped[str] = mapped_column(String, nullable=False)
    source_role_id: Mapped[str] = mapped_column(String, nullable=False)
    source_organization_id: Mapped[Optional[str]] = mapped_column(
        String,
        nullable=True,
    )
    ...
    status: Mapped[str] = mapped_column(String(32), nullable=False)
    quarantine_reason_code: Mapped[Optional[str]] = mapped_column(
        String(128),
        nullable=True,
    )
    resolved_tenant_id: Mapped[Optional[str]] = mapped_column(
        String,
        nullable=True,
    )
    resolved_scope_type: Mapped[Optional[str]] = mapped_column(
        String(64),
        nullable=True,
    )
    resolved_scope_id: Mapped[Optional[str]] = mapped_column(
        String,
        nullable=True,
    )
    canonical_binding_id: Mapped[Optional[str]] = mapped_column(
        String,
        nullable=True,
    )
...
Index(
    ...
    LegacyRoleBindingMigrationRecordORM.batch_id,
    LegacyRoleBindingMigrationRecordORM.status,
)
Index(
    "idx_legacy_role_binding_migration_reason",
    LegacyRoleBindingMigrationRecordORM.quarantine_reason_code,
)
...
class UserRoleORM(Base):
    # RBAC-TRANSITION-ONLY: Drop after CANONICAL_ONLY parity and rollback gates.
