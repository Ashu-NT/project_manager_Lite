...
class SqlAlchemyAccountingConnectorRepository(TenantScopedRepositorySupport):
    ...
    def get(self, *, for_update=False):
        ...
        return (
            AccountingConnectorConfiguration.model_validate(
                {
                    key: getattr(row, key)
                    for key in AccountingConnectorConfiguration.model_fields
                }
            )
            if row
            else None
    ...
    def save(self, configuration, *, expected_version):
        ...
        if (configuration.tenant_id, configuration.organization_id) != (
            scope.tenant_id,
            scope.organization_id,
        ):
            raise BusinessRuleError(
                "Connector scope mismatch.", code="INTEGRATION_SCOPE_VIOLATION"
            )
        values = configuration.model_dump()
        if expected_version is None:
            if configuration.version != 1:
                raise BusinessRuleError(
                    "Invalid connector version.", code="STALE_WRITE"
                )
            ...
            if result.rowcount != 1:
                raise BusinessRuleError(
                    "Connector changed concurrently.", code="STALE_WRITE"
                )
        self.session.flush()
