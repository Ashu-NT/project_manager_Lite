...
One dedicated read-side adapter -- not the write repository's ``list_for_organization`` fully
hydrating every ``Employee`` row just to compute two integers. ``EmployeeService`` depends on
the ``EmployeeHeadcountReader`` contract, never on this concrete class directly. Takes
``tenant_id``/``organization_id`` explicitly rather than resolving them from ambient session state.
"""
