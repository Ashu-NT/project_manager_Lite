...
def _legacy_float_text(value: float | None) -> str | None:
...
def _legacy_money_metric(code, label, value, currency, supporting_text):
    text = _legacy_float_text(value)
...
def _legacy_ratio_metric(code, label, value, supporting_text):
    text = _legacy_float_text(value)
    return FinancialPerformanceMetricDto(
        code=code,
        label=label,
...
def serialize_performance_evm(fact) -> FinancialEvmDto:
    ...
    return FinancialEvmDto(
        ...
        metrics=(
            _legacy_money_metric("bac", "Budget at Completion (BAC)", fact.bac, currency, "Cost-loaded baseline authority."),
            _legacy_money_metric("pv", "Planned Value (PV)", fact.pv, currency, "Baseline cost by planned completion at the as-of date."),
            _legacy_money_metric("ev", "Earned Value (EV)", fact.ev, currency, "Baseline cost weighted by authoritative task progress."),
            _legacy_money_metric("ac", "Actual Cost (AC)", fact.ac, currency, "Canonical posted Project Cost Entry authority."),
            _legacy_ratio_metric("cpi", "Cost Performance Index (CPI)", fact.cpi, "EV / AC; unavailable when AC is zero."),
            _legacy_ratio_metric("spi", "Schedule Performance Index (SPI)", fact.spi, "EV / PV; unavailable when PV is zero."),
            _legacy_money_metric("etc", "Estimate to Complete (ETC)", fact.etc, currency, "Approved Forecast ETC authority."),
            _legacy_money_metric("eac", "Estimate at Completion (EAC)", fact.eac, currency, "Existing authority: AC + approved Forecast ETC."),
            _legacy_money_metric("vac", "Variance at Completion (VAC)", fact.vac, currency, "Existing authority: BAC - EAC; positive is favorable."),
            _legacy_ratio_metric("tcpi_bac", "TCPI to BAC", fact.tcpi_bac, "Required cost efficiency to meet BAC."),
            _legacy_ratio_metric("tcpi_eac", "TCPI to EAC", fact.tcpi_eac, "Required cost efficiency to meet EAC."),
        ),
        notes=fact.notes,
    )
