...
class CommercialProjectionQuery:
    ...
    def read(
        ...
        *,
        ...
        project_id: str,
        as_of_date: date,
        include_profitability: bool,
    ) -> ProjectCommercialProjection:
        if not all((tenant_id, organization_id, project_id)) or not isinstance(
            as_of_date, date
        ):
        ...
        prepared = (
            self._billing_reader.approved_preparation_amount(
                tenant_id=tenant_id,
        ...
        state = (
            ...
            else Availability.RESTRICTED
        )
        reason = (
            (
                Reason.FINANCIAL_PROFILE_MISSING
                if financial is None
                else Reason.BILLING_PROFILE_MISSING
            )
            if include_profitability
            else Reason.PROFITABILITY_PERMISSION_REQUIRED
        )
        ...
        if include_profitability and financial is not None:
            method = (
                financial.billing_method
                if financial.is_billable
                else BillingMethod.NON_BILLABLE
            )
            # Unsupported methods do not need cost evidence or rate resolution.
            totals = (
                self._cost_totals(project_id, as_of_date)
                if profile is not None and method is BillingMethod.FIXED_PRICE
                else None
            )
            ...
            result = ProjectProfitabilityCalculator.calculate(
                ProfitabilityInputs(
                    ...
                    forecast_cost_at_completion=totals.estimate_at_completion
                    if totals
                    else None,
            ...
            revenue, margin, percent = (
                ...
                result.projected_margin_amount,
                result.projected_margin_percent,
            )
            basis = result.revenue_basis
            revenue_state, margin_state, percent_state = (
                result.revenue_availability,
                result.margin_availability,
                result.percent_availability,
            )
            revenue_reason, margin_reason, percent_reason = (
                result.revenue_reason,
                result.margin_reason,
                result.percent_reason,
            )
        ...
        return ProjectCommercialProjection(
            ...
            revenue_availability=revenue_state,
            margin_availability=margin_state,
