...
class SqlAlchemyPortfolioUnitOfWork(SqlAlchemyUnitOfWorkBase, PortfolioUnitOfWork):
    def __init__(
        self,
        *,
        session: Session,
        ...
        self.intake = SqlAlchemyPortfolioIntakeRepository(session)
        self.scenarios = SqlAlchemyPortfolioScenarioRepository(session)
        self.scoring_templates = SqlAlchemyPortfolioScoringTemplateRepository(session)
        self.dependencies = SqlAlchemyPortfolioProjectDependencyRepository(session)
        for repo in (self.intake, self.scenarios, self.scoring_templates, self.dependencies):
...
class SqlAlchemyPortfolioUnitOfWorkFactory(SqlAlchemyUnitOfWorkFactoryBase):
    def __init__(
        self,
        *,
    ...
    def create(self, *, context: DomainEventContext) -> SqlAlchemyPortfolioUnitOfWork:
        session = self._session_factory()
        configure_session_rls_context(session, user_session=self._user_session)
        return SqlAlchemyPortfolioUnitOfWork(
            session=session,
...
__all__ = ["SqlAlchemyPortfolioUnitOfWork", "SqlAlchemyPortfolioUnitOfWorkFactory"]
