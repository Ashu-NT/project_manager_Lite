...
class SqlAlchemyTaskUnitOfWork(SqlAlchemyUnitOfWorkBase, TaskUnitOfWork):
    def __init__(
        self,
        *,
        session: Session,
        ...
        self.dependencies = SqlAlchemyDependencyRepository(session)
        for repo in (self.tasks, self.assignments, self.dependencies):
...
class SqlAlchemyTaskUnitOfWorkFactory(SqlAlchemyUnitOfWorkFactoryBase):
    def __init__(
        self,
        *,
    ...
    def create(self, *, context: DomainEventContext) -> SqlAlchemyTaskUnitOfWork:
        session = self._session_factory()
        configure_session_rls_context(session, user_session=self._user_session)
        return SqlAlchemyTaskUnitOfWork(
            session=session,
...
__all__ = ["SqlAlchemyTaskUnitOfWork", "SqlAlchemyTaskUnitOfWorkFactory"]
