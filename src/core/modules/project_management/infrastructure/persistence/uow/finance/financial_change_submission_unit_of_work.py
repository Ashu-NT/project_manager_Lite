...
class SqlAlchemyFinancialChangeSubmissionUnitOfWork(
    SqlAlchemyUnitOfWorkBase, FinancialChangeSubmissionUnitOfWork
):
    def __init__(
        self,
        *,
        ...
        self.changes._tenant_context_service = tenant_context_service
        self.budgets = SqlAlchemyProjectBudgetRepository(session)
        self.budgets._tenant_context_service = tenant_context_service
        self.forecasts = SqlAlchemyProjectForecastRepository(session)
        self.forecasts._tenant_context_service = tenant_context_service
...
class SqlAlchemyFinancialChangeSubmissionUnitOfWorkFactory(SqlAlchemyUnitOfWorkFactoryBase):
    """Closes over a session *factory* -- every `create()` call opens a genuinely fresh
    `Session`, matching every other Platform/P5 canonical UoW factory's own convention."""
    ...
    def create(self, *, context: DomainEventContext) -> SqlAlchemyFinancialChangeSubmissionUnitOfWork:
        return SqlAlchemyFinancialChangeSubmissionUnitOfWork(
