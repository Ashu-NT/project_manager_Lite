...
class SqlAlchemyAccountingOutcomeConsumer:
    def __init__(self, *, session_factory, clock, transactional_dispatcher, post_commit_bus):
    ...
    def consume(self, ingress):
        ...
        with worker_tenant_scope(tenant_id=scope.tenant_id, organization_id=scope.organization_id):
            session = self._sessions()
            configure_session_rls_context(session, user_session=None)
            with SqlAlchemyFinanceGovernanceUnitOfWork(
                session=session, context=DomainEventContext(correlation_id=str(uuid4())),
                transactional_dispatcher=self._dispatcher, post_commit_bus=self._bus,
                tenant_context_service=scope, user_session=None,
            ) as uow:
                principal = resolve_execution_principal(
                    name=identity.principal_name, scope=scope,
                    principal_repository=SqlAlchemyServicePrincipalRepository(session, tenant_context_service=scope),
                    user_repository=SqlAlchemyUserRepository(session),
                )
                if session.get_bind().dialect.name == "postgresql":
                    session.execute(text("SELECT set_config('app.user_id', :actor, true)"), {"actor": principal.user_id})
                # One org connector lock serializes duplicate ingress, including first insert.
                config = uow.accounting_connectors.get(for_update=True)
                if config is None or not config.enabled or (config.adapter_id, config.connection_id) != (identity.adapter_id, identity.connection_id):
                    raise AccountingIngressAuthenticationError()
                with worker_tenant_scope(tenant_id=scope.tenant_id, organization_id=scope.organization_id, actor_user_id=principal.user_id):
                    result = self._consume(session, uow, ingress, principal.user_id)
                    uow.commit()
                    return result

    def _quarantine(self, uow, ingress, actor, reason, *, project_id=None):
        receipt = AccountingOutcomeQuarantine(repository=uow.finance_inbox, clock=self._clock).record(ingress, reason=reason)
        self._audit(uow, ingress, actor, result=reason.value, entity_id=receipt.id, project_id=project_id)
        return "quarantined"
    ...
    def _consume(self, session, uow, ingress, actor):
        ...
        inbox = IntegrationInboxService(repository=uow.finance_inbox, consumer_name=ACCOUNTING_OUTCOME_CONSUMER, clock=self._clock)
        ...
        if decision.disposition is not InboxDeliveryDisposition.READY:
            return self._quarantine(uow, ingress, actor, OutcomeRejectionReason.EVENT_CONTENT_CONFLICT)
        identity = ingress.connection
        handoff = session.scalar(select(ProjectAccountingHandoffORM).where(
        ...
        if handoff is None:
            ...
            row = session.scalar(select(ProjectAccountingOutboxORM).where(
                ProjectAccountingOutboxORM.event_id == handoff.id,
                ProjectAccountingOutboxORM.tenant_id == identity.tenant_id,
                ProjectAccountingOutboxORM.organization_id == identity.organization_id,
                ProjectAccountingOutboxORM.project_id == project_id,
            ).with_for_update())
            preparation = uow.billing.get_preparation(handoff.preparation_id, for_update=True)
            if row is None or (row.target_adapter_id, row.target_connection_id) != (identity.adapter_id, identity.connection_id):
                reason = OutcomeRejectionReason.CONNECTOR_MISMATCH
            elif preparation is None or preparation.project_id != project_id or handoff.approved_version != outcome.approved_source_version or handoff.payload_hash != outcome.handoff_payload_hash:
                ...
                previous_sequence = uow.finance_inbox.latest_processed_aggregate_version(
                    consumer_name=ACCOUNTING_OUTCOME_CONSUMER,
                    aggregate_type="accounting_handoff_outcome", aggregate_id=aggregate_id,
                )
                reason = validate_outcome_transition(
                    previous=previous, incoming=outcome.outcome, previous_sequence=previous_sequence,
                    incoming_sequence=outcome.sequence, authoritative_sequence=identity.authoritative_sequence,
                    transport_delivered=row.status == "published" and row.transport_receipt_json is not None,
                )
        if reason is not None:
            inbox.quarantine(decision.receipt.id, reason_code=reason.value, message="Accounting outcome correlation or transition is invalid.")
        ...
        event = ProjectBillingExternalEvent.create(
            tenant_id=identity.tenant_id, organization_id=identity.organization_id,
            project_id=project_id, preparation_id=preparation.id, event_type=kind,
            external_system="external_accounting", external_status=outcome.outcome.value,
            idempotency_key=envelope.event_id, occurred_at=outcome.occurred_at,
            reconciliation_reference=outcome.reconciliation_reference, recorded_at=self._clock.now(),
        )
        ...
        self._audit(uow, ingress, actor, result=outcome.outcome.value, entity_id=handoff.id, project_id=project_id)
        uow.record_event(BillingPreparationExternalOutcomeRecorded(
            tenant_id=identity.tenant_id, organization_id=identity.organization_id,
            project_id=project_id, billing_preparation_id=preparation.id,
            external_event_id=event.id, event_type=kind, occurred_at=self._clock.now(),
        ))
        return "processed"

    @staticmethod
    def _previous(session, preparation):
        rejected = session.scalar(select(ProjectBillingExternalEventORM.id).where(
            ProjectBillingExternalEventORM.tenant_id == preparation.tenant_id,
            ProjectBillingExternalEventORM.organization_id == preparation.organization_id,
            ...
            ProjectBillingExternalEventORM.event_type == BillingExternalEventType.DELIVERY_REJECTED.value,
        ).limit(1))
        if rejected is not None:
            return AccountingOutcomeKind.REJECTED
        return {"acknowledged": AccountingOutcomeKind.ACKNOWLEDGED, "reconciled": AccountingOutcomeKind.RECONCILED}.get(preparation.status.value)

    @staticmethod
    def _audit(uow, ingress, actor, *, result, entity_id, project_id):
        identity = ingress.connection
        uow._enterprise_audit_service.record(
            operation="accounting_outcome.consume", entity_type="AccountingHandoff",
            entity_id=entity_id, entity_parent_id=project_id, module="project_management",
            category="FINANCIAL", actor_id=actor, actor_type="service", source="ACCOUNTING_INGRESS",
            project_id=project_id, commit=False,
            after_data={"adapter_id": identity.adapter_id, "connection_id": identity.connection_id,
                        "result": result, "body_sha256": ingress.body_sha256,
                        "event_id": ingress.outcome.external_event_id if ingress.outcome else None,
