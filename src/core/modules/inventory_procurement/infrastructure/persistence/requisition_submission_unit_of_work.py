...
from src.core.modules.inventory_procurement.contracts.persistence.requisition_submission_unit_of_work import (
    RequisitionSubmissionUnitOfWork,
)
...
class SqlAlchemyRequisitionSubmissionUnitOfWork(SqlAlchemyUnitOfWorkBase, RequisitionSubmissionUnitOfWork):
    def __init__(
        self,
        *,
        session: Session,
        ...
        self.requisitions = SqlAlchemyPurchaseRequisitionRepository(
            session, tenant_context_service=tenant_context_service
        )
        self.requisition_lines = SqlAlchemyPurchaseRequisitionLineRepository(
            session, tenant_context_service=tenant_context_service
        )
...
class SqlAlchemyRequisitionSubmissionUnitOfWorkFactory(SqlAlchemyUnitOfWorkFactoryBase):
    def __init__(
        self,
        *,
    ...
    def create(self, *, context: DomainEventContext) -> SqlAlchemyRequisitionSubmissionUnitOfWork:
        return SqlAlchemyRequisitionSubmissionUnitOfWork(
...
__all__ = [
    "SqlAlchemyRequisitionSubmissionUnitOfWork",
