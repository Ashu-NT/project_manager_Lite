...
class PurchasingSupportMixin:
    def _validate_source_requisition(
        ...
        if requisition is None or requisition.organization_id != organization_id:
            raise ValidationError(
                "Source requisition was not found in the active organization.",
                code="INVENTORY_REQUISITION_SCOPE_INVALID",
            )
        if requisition.status not in {
            ...
            raise ValidationError(
                "Source requisition is not ready for purchasing.",
    ...
    def _validate_source_requisition_line(
        ...
        *,
        ...
        if purchase_order.source_requisition_id and requisition_line.purchase_requisition_id != purchase_order.source_requisition_id:
            raise ValidationError(
                "Source requisition line does not belong to the purchase-order requisition.",
                code="INVENTORY_REQUISITION_LINE_SCOPE_INVALID",
            )
        remaining = max(
            0.0,
            float(requisition_line.quantity_requested or 0.0) - float(requisition_line.quantity_sourced or 0.0),
        )
        if remaining <= 0:
            raise ValidationError(
                "Source requisition line is already fully sourced.",
                code="INVENTORY_REQUISITION_LINE_FULLY_SOURCED",
            )
        ...
        if ordered_in_requisition_uom > remaining:
            raise ValidationError(
                "Purchase-order quantity exceeds the remaining requisition demand.",
                code="INVENTORY_REQUISITION_LINE_QTY_EXCEEDED",
            )
    ...
    def _require_draft_purchase_order(self, purchase_order_id: str) -> PurchaseOrder:
        ...
        if purchase_order.status != PurchaseOrderStatus.DRAFT:
            raise ValidationError(
                ...
                code="INVENTORY_PURCHASE_ORDER_NOT_DRAFT",
            )
    ...
    def _require_requisition_line(self, line_id: str) -> PurchaseRequisitionLine:
        ...
        if line is None:
            raise ValidationError(
                "Source requisition line not found.",
                code="INVENTORY_REQUISITION_LINE_NOT_FOUND",
            )
        return line
