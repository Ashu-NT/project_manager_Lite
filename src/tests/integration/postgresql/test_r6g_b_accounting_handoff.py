...
@pytest.fixture
def handoff_scope(postgres_test_environment, request):
    scope = request.getfixturevalue("billing_scope")
    ...
    with postgres_test_environment.admin_engine.begin() as connection:
        for actor in actors:
            connection.execute(
                text(
                    "INSERT INTO users (id, username, password_hash, account_type, is_active, created_at, updated_at, version) VALUES (:id, :id, 'no-login', 'human', true, :now, :now, 1)"
                ),
                {"id": actor, "now": now},
            )
        connection.execute(
            text(
                "UPDATE project_billing_profiles SET customer_party_id='customer-reference' WHERE project_id=:project"
            ),
            {"project": scope.project},
        )
        connection.execute(
            text(
                "UPDATE project_billing_preparations SET approved_by=:actor, approved_at=:now, approval_request_id='approved-request' WHERE id=:id"
            ),
            {"actor": actors[0], "now": now, "id": scope.preparation},
        )
        connection.execute(
            text(
                "INSERT INTO project_finance_profiles (id, tenant_id, organization_id, project_id, currency_code, billing_method, is_billable, created_at, updated_at) VALUES (:project, :tenant, :org, :project, 'USD', 'fixed_price', true, :now, :now)"
            ),
            {
                "project": scope.project,
                "tenant": scope.tenant,
        ...
        connection.execute(
            text(
                "INSERT INTO organization_module_entitlements (tenant_id, organization_id, module_code, licensed, enabled, lifecycle_status, updated_at) VALUES (:tenant, :org, 'accounting_integration', true, true, 'active', :now)"
            ),
            {"tenant": scope.tenant, "org": scope.org, "now": now},
        )
        connection.execute(
            text(
                "INSERT INTO organization_accounting_connectors (tenant_id, organization_id, adapter_id, connection_id, secret_reference, enabled, version) VALUES (:tenant, :org, 'test_connector', 'test_connection', 'test_secret_ref', true, 1)"
            ),
            {"tenant": scope.tenant, "org": scope.org},
        )
    scope.actors = actors
    return scope
...
def boundary(environment, scope, actor, *, barrier=None):
    ...
    factory = SqlAlchemyFinanceGovernanceUnitOfWorkFactory(
        session_factory=sessionmaker(
            bind=environment.runtime_engine, expire_on_commit=False
        ),
...
def request(boundary, scope):
    return boundary.billing_preparation(
        lambda service: service.request_delivery(
            scope.preparation, expected_row_version=1
        )
    )


def test_two_authorized_users_converge_in_independent_runtime_uows(
    postgres_test_environment, handoff_scope
):
    scope = handoff_scope
    barrier = Barrier(2)
    ...
    def writer(actor):
        return request(
            boundary(postgres_test_environment, scope, actor, barrier=barrier), scope
        ).handoff_id
    ...
    with postgres_test_environment.runtime_session(
        ...
        assert (
            session.scalar(
                select(func.count()).select_from(ProjectAccountingHandoffORM)
            )
            == 1
        )
        assert (
        ...
        assert (
            session.scalar(
                text("SELECT version FROM project_billing_preparations WHERE id=:id"),
...
def test_runtime_uow_rolls_back_and_retries(
    postgres_test_environment, handoff_scope, monkeypatch, failure
):
...
def test_handoff_scope_and_database_identity_constraints(
    postgres_test_environment, handoff_scope
):
    scope = handoff_scope
    ...
    with postgres_test_environment.runtime_session(
        ...
        assert (
            repository.get_for_preparation("wrong-project", scope.preparation) is None
        )
        for changes in (
            {"id": str(uuid4())},
        ...
        for table, values in (
            ...
            with pytest.raises(DBAPIError, match="immutable"):
                with session.begin_nested():
                    session.execute(update(table).values(**values))
    for tenant, org in (
        ("other-tenant", scope.org),
        (scope.tenant, "other-org"),
        (None, None),
    ):
        with postgres_test_environment.runtime_session(
            ...
            assert (
                session.scalar(
                    select(func.count()).select_from(ProjectAccountingOutboxORM)
                )
                == 0
            )
            with pytest.raises(DBAPIError) as denied:
                session.execute(
                    insert(ProjectAccountingHandoffORM).values(
                        **(saved | {"id": str(uuid4())})
                    )
                )
...
@pytest.mark.parametrize("timing", ["before_lock", "after_lock"])
def test_profile_edit_cannot_mix_snapshot_versions(
    postgres_test_environment, handoff_scope, monkeypatch, timing
):
    ...
    original = SqlAlchemyProjectBillingRepository.get_profile

    def coordinated(repository, project_id, *, for_update=False):
        if not for_update:
            return original(repository, project_id)
        result = (
            original(repository, project_id, for_update=True)
            if timing == "after_lock"
            else None
        )
        reached.set()
        ...
        return (
            ...
            else original(repository, project_id, for_update=True)
        )

    monkeypatch.setattr(SqlAlchemyProjectBillingRepository, "get_profile", coordinated)
        ...
        try:
            with postgres_test_environment.runtime_session(
                ...
                def edit():
                    session.execute(
                        text(
                            "UPDATE project_billing_profiles SET contract_reference='REVISED', payment_terms_days=99, version=2 WHERE project_id=:id"
                        ),
                        {"id": scope.project},
                    )
                    session.commit()
    ...
    with postgres_test_environment.runtime_session(
        ...
        snapshot = AccountingHandoffSnapshot.model_validate_json(
            session.scalars(select(ProjectAccountingHandoffORM.payload_bytes)).one()
        )
        if timing == "before_lock":
            assert snapshot.billing_profile_version == 2
            assert snapshot.evidence.contract_reference == "REVISED"
            assert snapshot.evidence.payment_terms_days == 99
        else:
            assert snapshot.billing_profile_version == 1
            assert snapshot.evidence.payment_terms_days == 30


@pytest.mark.parametrize("changed", ["version", "approval"])
def test_preparation_change_before_lock_fails_closed(
    postgres_test_environment, handoff_scope, changed
):
    ...
    with postgres_test_environment.runtime_session(
        ...
        assignment = "version=2" if changed == "version" else "status='submitted'"
        session.execute(
            text(f"UPDATE project_billing_preparations SET {assignment} WHERE id=:id"),
            {"id": scope.preparation},
        )
        session.commit()
    ...
    assert error.value.code == (
        "STALE_WRITE" if changed == "version" else "business_precondition_failed"
    )
    with postgres_test_environment.runtime_session(
        tenant_id=scope.tenant, organization_id=scope.org
    ) as session:
