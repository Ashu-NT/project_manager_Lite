"""Phase I closeout visual QA: scene-graph offscreen capture of the two-level
navigation shell at multiple breakpoints and themes. Not a correctness
assertion suite -- renders PNGs to a scratch directory for manual visual
inspection, and asserts only that rendering itself completes without error."""
...
def _build_shell(services, *, theme_mode: str):
    ...
    return engine, root, shell_context, platform_catalog, pm_catalog


@pytest.mark.parametrize("theme_mode", ["light", "dark"])
def test_capture_navigation_shell_screenshots(qapp, services, theme_mode) -> None:
    ...
    try:
        for size_name, (w, h) in SIZES.items():
            engine, root, shell_context, platform_catalog, pm_catalog = _build_shell(
            ...
            # -- Overview: global sidebar only ---------------------------
            saved = _grab(qapp, item, OUT_DIR / f"overview_{theme_mode}_{size_name}.png")
            assert saved, f"overview_{theme_mode}_{size_name} failed to save"
