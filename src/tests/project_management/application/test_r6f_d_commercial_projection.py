...
def inputs(**changes):
    return replace(ProfitabilityInputs(
        billing_method=BillingMethod.FIXED_PRICE, contract_value=Decimal("1000000"),
        forecast_cost_at_completion=Decimal("750000"), project_currency="XAF",
        contract_currency="XAF", cost_currency="XAF",
    ), **changes)


@pytest.mark.parametrize("eac,margin,percent", [
    ("750000", "250000", "25"), ("1000000", "0", "0"),
    ("1250000", "-250000", "-25"),
])
def test_exact_golden_scenarios(eac, margin, percent):
    result = ProjectProfitabilityCalculator.calculate(inputs(forecast_cost_at_completion=Decimal(eac)))
...
def test_missing_unsupported_and_currency_are_not_zero(changes, state):
    ...
    if "forecast_cost_at_completion" in changes or changes.get("cost_currency") == "EUR":
        assert result.forecast_revenue_at_completion == Decimal("1000000")
...
def test_zero_denominator_and_decimal_precision():
    ...
    precise = ProjectProfitabilityCalculator.calculate(inputs(
        contract_value=Decimal("1.000001"), forecast_cost_at_completion=Decimal("0.000001"),
    ))
    assert precise.projected_margin_amount == Decimal("1.000000")
    assert precise.projected_margin_percent == Decimal("1.000000") / Decimal("1.000001") * Decimal(100)
...
def query_fixture(*, method=BillingMethod.FIXED_PRICE, foreign=None):
    ...
    financial = SimpleNamespace(**scope, billing_method=method, is_billable=True, currency_code="XAF")
    calls = []
    prepared = [Decimal(100)]

    def totals(project, cutoff):
        calls.append((project, cutoff))
        return SimpleNamespace(project_id=project, project_currency="XAF", estimate_at_completion=Decimal(75))
    ...
    query = CommercialProjectionQuery(
        ...
        billing_reader=SimpleNamespace(approved_preparation_amount=lambda **kw: prepared[0]),
        cost_totals=totals,
    )
    return query, dict(**scope, as_of_date=date(2026, 8, 31), include_profitability=True), calls, prepared
...
@pytest.mark.parametrize("method", [BillingMethod.TIME_AND_MATERIALS, BillingMethod.COST_PLUS, BillingMethod.NON_BILLABLE])
...
def test_preparation_progress_does_not_become_revenue_and_fact_is_immutable():
    ...
    assert first.forecast_revenue_at_completion == second.forecast_revenue_at_completion == Decimal(100)
    assert first.projected_margin_amount == second.projected_margin_amount == Decimal(25)
...
@pytest.mark.parametrize("model", [ProfitabilityResult, ProjectCommercialProjection, FinancialCommercialProjectionDto])
...
@pytest.mark.parametrize("contract,eac,expected_percent", [
    ("0", "0", None), ("100000", "100000", Decimal(0)),
])
def test_zero_metrics_are_typed_available_and_serialized(contract, eac, expected_percent):
    calculated = ProjectProfitabilityCalculator.calculate(inputs(
        contract_value=Decimal(contract), forecast_cost_at_completion=Decimal(eac),
    ))
    query, request, _, _ = query_fixture()
    fact = replace(query.read(**request),
        ...
        revenue_reason=calculated.revenue_reason, margin_reason=calculated.margin_reason,
...
@pytest.mark.parametrize("state,reason,label", [
    (Availability.AVAILABLE, None, "Available"),
    (Availability.NOT_CONFIGURED, Reason.BILLING_PROFILE_MISSING, "Not configured"),
    (Availability.UNSUPPORTED, Reason.T_AND_M_FORECAST_AUTHORITY_MISSING, "Projection unavailable for this billing method"),
    (Availability.UNAVAILABLE, Reason.EAC_UNAVAILABLE, "Canonical EAC unavailable"),
    (Availability.RESTRICTED, Reason.PROFITABILITY_PERMISSION_REQUIRED, "Restricted"),
...
def test_serialization_and_presentation_preserve_each_state(state, reason, label):
    query, request, _, _ = query_fixture()
    result = replace(query.read(**request),
        revenue_availability=state, margin_availability=state, percent_availability=state,
...
def test_missing_eac_uses_unavailable_not_unsupported_or_restricted():
    result = ProjectProfitabilityCalculator.calculate(inputs(forecast_cost_at_completion=None))
