...
def _seed_priority_pm_rows(services):
    ...
    task_a1 = TaskORM(id="task-a-1", project_id=project_a.id, name="Task A1", status=TaskStatus.TODO, version=1)
    task_a2 = TaskORM(id="task-a-2", project_id=project_a.id, name="Task A2", status=TaskStatus.TODO, version=1)
    task_b1 = TaskORM(id="task-b-1", project_id=project_b.id, name="Task B1", status=TaskStatus.TODO, version=1)
    task_b2 = TaskORM(id="task-b-2", project_id=project_b.id, name="Task B2", status=TaskStatus.TODO, version=1)
...
def test_priority_pm_repositories_hide_other_organization_rows(services):
    ...
    assert [row.id for row in assignment_repo.list_by_tasks([seeded["task_a1"], seeded["task_b1"]])] == [seeded["assignment_a"]]
    ...
    assert [row.id for row in comment_repo.list_recent_for_tasks([seeded["task_a1"], seeded["task_b1"]])] == [seeded["comment_a"]]
