...
class _FakeCollaborationService:
    ...
    def list_task_presence(self, task_id: str) -> list[SimpleNamespace]:
        ...
        return [
            SimpleNamespace(
                task_id="task-1", task_name="Cable Pull", project_id="proj-1", project_name="Plant Upgrade",
                username="planner", display_name="Alex Taylor", activity="reviewing",
                last_seen_at=datetime(2026, 5, 1, 9, 35), is_self=True,
            )
        ]
    ...
    def post_comment(self, *, task_id, body, attachments=(), linked_document_ids=(), parent_comment_id=None) -> SimpleNamespace:
        ...
        comment = SimpleNamespace(
            id="comment-posted-1", task_id=task_id, author_username="alex", body=body,
            mentions=["planner"], attachments=list(attachments), created_at=datetime(2026, 5, 1, 10, 15),
        )
        ...
        self._comment_documents[comment.id] = [
            SimpleNamespace(
                ...
                storage_uri="ticket-456", file_name="",
            )
        ]
        return comment
...
class _FakeTaskTimesheetsDesktopApi:
    ...
    def build_assignment_snapshot(self, assignment_id, *, period_start=None):
        ...
        entry_rows = [
            SimpleNamespace(entry_id="entry-1", entry_date_label="2026-05-03", hours=6.0, hours_label="6.00h", note="Cable tray installation", author_username="alex"),
            *[
                SimpleNamespace(
                    entry_id=row["entry_id"], entry_date_label=row["entry_date"],
                    hours=row["hours"], hours_label=f"{float(row['hours']):.2f}h",
                    note=row["note"], author_username="alex",
                )
                for row in self.added_entries
            ],
        ]
        total_hours = sum(float(e.hours or 0.0) for e in entry_rows)
        return SimpleNamespace(
            assignment=SimpleNamespace(value="assign-1", label="Plant Upgrade | Cable Pull | Alex Taylor", project_id="proj-1"),
            period_options=(SimpleNamespace(value="2026-05-01", label="May 2026"), SimpleNamespace(value="2026-04-01", label="Apr 2026")),
            selected_period_start=selected_period_start,
            period_summary=SimpleNamespace(
                period_id="period-1", period_start_label="May 2026", period_end_label="2026-05-31",
                status="OPEN", status_label="Open", resource_id="res-1", resource_name="Alex Taylor",
                total_hours_label=f"{total_hours:.2f}h", entry_count=len(entry_rows),
                submitted_by_username="-", submitted_at_label="-", decided_by_username="-", decided_at_label="-", decision_note="",
            ),
    ...
    def add_time_entry(self, command):
        ...
        self.added_entries.append({"entry_id": entry_id, "entry_date": command.entry_date.isoformat(), "hours": float(command.hours), "note": command.note or ""})
    ...
    def delete_time_entry(self, entry_id):
        self.added_entries = [r for r in self.added_entries if r["entry_id"] != entry_id]
...
def _make_task_service() -> _FakeTaskService:
    task_service = _FakeTaskService(
        [
            _build_task_record(task_id="task-1", project_id="proj-1", name="Cable Pull", description="Primary feeder cable installation.", status=TaskStatus.IN_PROGRESS, start_date=date(2026, 5, 3), end_date=date(2026, 5, 6), duration_days=4, priority=70, percent_complete=45.0, deadline=date(2026, 5, 7)),
            _build_task_record(task_id="task-2", project_id="proj-1", name="Punchlist Closeout", description="Commissioning closeout walkdown.", status=TaskStatus.BLOCKED, start_date=date(2026, 5, 8), end_date=date(2026, 5, 9), duration_days=2, priority=95, percent_complete=0.0, deadline=date(2026, 5, 9)),
            _build_task_record(task_id="task-3", project_id="proj-2", name="Lighting Retrofit", description="Warehouse fixture replacement.", status=TaskStatus.TODO, start_date=date(2026, 5, 10), end_date=date(2026, 5, 12), duration_days=3, priority=40, percent_complete=0.0, deadline=date(2026, 5, 13)),
            _build_task_record(task_id="task-4", project_id="proj-1", name="As-Built Handover", description="Finalize turnover package and as-built dossier.", status=TaskStatus.DONE, start_date=date(2026, 5, 10), end_date=date(2026, 5, 10), duration_days=1, priority=50, percent_complete=100.0, deadline=date(2026, 5, 10)),
        ]
    )
    task_service.register_project_resource("pr-1", "res-1")
    task_service.assign_project_resource(task_id="task-1", project_resource_id="pr-1", allocation_percent=55.0)
    task_service.add_dependency(predecessor_id="task-1", successor_id="task-2", dependency_type="FS", lag_days=2)
    return task_service
