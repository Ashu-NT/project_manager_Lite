...
def test_maintenance_root_repositories_hide_cross_organization_rows(services) -> None:
    ...
    task_template_repo = _maintenance_repo(
        SqlAlchemyMaintenanceTaskTemplateRepository,
        services,
    )
    ...
    preventive_plan_repo = _maintenance_repo(
        SqlAlchemyMaintenancePreventivePlanRepository,
        services,
    )
    ...
    assert location_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_location_code"],
    ) is None
    assert system_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_system_code"],
    ) is None
    assert asset_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_asset_code"],
    ) is None
    assert sensor_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_sensor_code"],
    ) is None
    assert integration_source_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_integration_code"],
    ) is None
    assert failure_code_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_failure_code"],
    ) is None
    assert task_template_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_task_template_code"],
    ) is None
    assert work_request_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_work_request_code"],
    ) is None
    assert work_order_repo.get_by_code(
        seeded["other_org_id"],
        seeded["current_work_order_code"],
    ) is None
    assert preventive_plan_repo.get_by_code(
        seeded["other_org_id"],
    ...
    current_work_request_ids = {
        row.id
        for row in work_request_repo.list_for_organization(
            seeded["current_org_id"],
        )
    }
    current_work_order_ids = {
        row.id
        for row in work_order_repo.list_for_organization(
            seeded["current_org_id"],
        )
    }
    ...
    assert location_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    assert system_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    assert asset_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    assert sensor_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    assert integration_source_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    assert failure_code_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    assert task_template_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
    ...
    assert preventive_plan_repo.list_for_organization(
        seeded["other_org_id"],
        active_only=None,
    ) == []
