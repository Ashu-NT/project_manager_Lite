...
def test_legacy_project_scoped_grant_no_longer_grants_authority(services):
    ...
    project = project_service.create_project("Phase 2B Legacy Project")
    user = auth.register_user(
        "p2b-legacy-project-scope",
        "StrongPass123!",
        role_names=["viewer"],
        tenant_id=tenant_id,
    )
    ...
    assert project.id not in principal.scoped_access.get("project", {})
    assert "project.manage" not in principal.permissions
    assert project.id not in principal.project_access


def test_legacy_project_membership_no_longer_grants_authority(services):
    from src.core.platform.access.domain import ProjectMembership

    auth = services["auth_service"]
    project_service = services["project_service"]
    tenant_id, _ = _active_context_ids(services)

    project = project_service.create_project("Phase 2B Legacy Membership Project")
    user = auth.register_user(
        "p2b-legacy-project-membership",
        "StrongPass123!",
        role_names=["viewer"],
        tenant_id=tenant_id,
    )
    ...
    auth._project_membership_repo.add(membership)
    services["session"].flush()
