...
def test_pm_resources_workspace_still_reacts_to_resources_and_employees(services):
    """Confirms the resources binder's `calendars_changed` removal did not accidentally also
    remove its other, still-real subscriptions."""
    pm_catalog = _pm_catalog(services)
    ...
    domain_events.employees_changed.emit(_unique("p7b-employee"))
