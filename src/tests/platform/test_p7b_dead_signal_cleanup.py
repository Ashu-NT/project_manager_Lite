...
def test_admin_console_still_reacts_to_its_remaining_five_real_signals(services):
    """P10D/P12B: `organizations_changed`/`employees_changed` are gone (both now flow through
    their own typed ViewInvalidation targets, wired directly in `context.py`, not through this
    composite Signal list) -- five legacy signals remain here."""
    catalog = _catalog(services)
...
def test_domain_event_binder_still_kept_unchanged_in_responsibility():
    ...
    coordination, now for 5 signals instead of 8 (`calendars_changed` removed by P7B,
    `organizations_changed` removed by P10D, `employees_changed` removed by P12B -- both route
    ...
    for forbidden in (
        ...
        "organizations_changed", "employees_changed",
    ):
