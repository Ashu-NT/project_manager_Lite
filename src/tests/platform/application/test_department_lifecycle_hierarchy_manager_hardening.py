...
self-parenting; and Manager/Lead fully wired from the dialog through the
presenter's create/update payload mapping and read-model name resolution."""
...
# --------------------------------------------------------------------------
# Manager / Lead: presenter payload mapping + read-model name resolution
# --------------------------------------------------------------------------


def test_create_department_with_manager_via_presenter_payload(services) -> None:
    employee_service = services["employee_service"]
    registry = build_desktop_api_registry(services)
    catalog = PlatformWorkspaceCatalog(desktop_api_registry=registry)
    admin = catalog.adminWorkspace

    manager = employee_service.create_employee(employee_code="MGR-1", full_name="Anna Mueller")

    result = admin.createDepartment({
        "departmentCode": "MGR-DEPT-1",
    ...
    row = next(item for item in catalog_result["items"] if item["state"]["departmentCode"] == "MGR-DEPT-1")
    assert row["state"]["managerEmployeeId"] == manager.id
    assert row["state"]["managerDisplay"] == "Anna Mueller"
    # Never a raw UUID standing in for the display name.
    assert row["state"]["managerDisplay"] != manager.id


def test_update_department_manager_via_presenter_payload(services) -> None:
    department_service = services["department_service"]
    employee_service = services["employee_service"]
    registry = build_desktop_api_registry(services)
    catalog = PlatformWorkspaceCatalog(desktop_api_registry=registry)
    ...
    department = department_service.create_department(department_code="MGR-DEPT-2", name="Dept Two")
    manager = employee_service.create_employee(employee_code="MGR-2", full_name="John Doe")
    ...
    assert reloaded.manager_employee_id == manager.id


def test_clear_department_manager_via_presenter_payload(services) -> None:
    department_service = services["department_service"]
    employee_service = services["employee_service"]
    registry = build_desktop_api_registry(services)
    catalog = PlatformWorkspaceCatalog(desktop_api_registry=registry)
    ...
    manager = employee_service.create_employee(employee_code="MGR-3", full_name="Clear Me")
    department = department_service.create_department(
        department_code="MGR-DEPT-3", name="Dept Three", manager_employee_id=manager.id
    )
    assert department.manager_employee_id == manager.id
...
def test_cross_organization_manager_still_rejected(services) -> None:
    department_service = services["department_service"]
    employee_service = services["employee_service"]
    organization_service = services["organization_service"]
    tenant_context_service = services["tenant_context_service"]
    ...
    other_organization = organization_service.create_organization(
        organization_code="MGR-CROSSORG", display_name="Other Org", timezone_name="UTC", base_currency="USD"
    )
    tenant_context_service.set_active_organization(other_organization.id)
    foreign_employee = employee_service.create_employee(employee_code="MGR-FOREIGN", full_name="Foreign Manager")
    tenant_context_service.set_active_organization(default_organization.id)

    with pytest.raises(ValidationError, match="Department manager employee does not exist"):
        department_service.create_department(
            department_code="MGR-CROSSORG-DEPT", name="Cross Org Dept", manager_employee_id=foreign_employee.id
        )


def test_manager_options_are_real_employee_names_scoped_to_active_organization(services) -> None:
    employee_service = services["employee_service"]
    registry = build_desktop_api_registry(services)
    catalog = PlatformWorkspaceCatalog(desktop_api_registry=registry)
    admin = catalog.adminWorkspace

    employee_service.create_employee(employee_code="MGR-OPT-1", full_name="Option Employee One")
    admin.refresh()

    options = admin.departmentEditorOptions["managerOptions"]
