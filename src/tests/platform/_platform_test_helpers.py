...
def _module(*, code: str, label: str, description: str, stage: str = "active") -> ModuleDto:
    return ModuleDto(
        code=code,
        label=label,
...
class FakePlatformRuntimeApi:
    def __init__(self) -> None:
        self._organizations: list[OrganizationDto] = [
            _organization(organization_id="org-1", code="ORG", display_name="TechAsh"),
            _organization(organization_id="org-2", code="OPS", display_name="Operations", is_active=False),
        ]
        self._modules = (
...
class FakePlatformSiteApi:
    def __init__(self, *, runtime_api: FakePlatformRuntimeApi, rows: tuple[SiteDto, ...]) -> None:
...
class FakePlatformDepartmentApi:
    ...
    def list_departments(self, *, active_only: bool | None = None) -> DesktopApiResult[tuple[DepartmentDto, ...]]:
        rows = self._rows
        if active_only is not None:
            rows = [row for row in rows if row.is_active == active_only]
        return DesktopApiResult(ok=True, data=tuple(rows))
    ...
    def update_employee(self, command) -> DesktopApiResult[EmployeeDto]:
        for index, row in enumerate(self._rows):
            ...
            updated = replace(
                ...
                employment_type=row.employment_type if command.employment_type is None else command.employment_type,
        ...
        return DesktopApiResult(
            ok=False,
            error=DesktopApiError(code="employee_not_found", message=f"Employee '{command.employee_id}' was not found.", category="not_found"),
        )


class FakePlatformUserApi:
    ...
    def create_user(self, command) -> DesktopApiResult[UserDto]:
        if any(row.username == command.username for row in self._rows):
            return DesktopApiResult(
                ok=False,
                error=DesktopApiError(code="user_exists", message=f"User '{command.username}' already exists.", category="conflict"),
            )
        user = UserDto(
    ...
    def update_user(self, command) -> DesktopApiResult[UserDto]:
        ...
        return DesktopApiResult(
            ok=False,
            error=DesktopApiError(code="user_not_found", message=f"User '{command.user_id}' was not found.", category="not_found"),
        )
    ...
    def set_user_active(self, user_id: str, is_active: bool) -> DesktopApiResult[UserDto]:
        ...
        return DesktopApiResult(
            ok=False,
            error=DesktopApiError(code="user_not_found", message=f"User '{user_id}' was not found.", category="not_found"),
        )
    ...
    def _update_roles(self, user_id: str, *, add: str | None = None, remove: str | None = None) -> DesktopApiResult[UserDto]:
        for index, row in enumerate(self._rows):
            if row.id != user_id:
                continue
...
class FakePlatformPartyApi:
    def __init__(self, *, runtime_api: FakePlatformRuntimeApi, rows: tuple[PartyDto, ...]) -> None:
        self._runtime_api = runtime_api
        self._rows = list(rows)
    ...
    def list_parties(self, *, active_only: bool | None = None) -> DesktopApiResult[tuple[PartyDto, ...]]:
        rows = self._rows
        if active_only is not None:
            rows = [row for row in rows if row.is_active == active_only]
        return DesktopApiResult(ok=True, data=tuple(rows))
...
class FakePlatformDocumentApi:
    def __init__(self, *, runtime_api: FakePlatformRuntimeApi, rows: tuple[DocumentDto, ...], structure_rows: tuple[DocumentStructureDto, ...], link_rows: tuple[DocumentLinkDto, ...] = ()) -> None:
        self._runtime_api = runtime_api
        self._rows = list(rows)
    ...
    def list_documents(self, *, active_only: bool | None = None) -> DesktopApiResult[tuple[DocumentDto, ...]]:
        rows = self._rows
        if active_only is not None:
            rows = [row for row in rows if row.is_active == active_only]
        return DesktopApiResult(ok=True, data=tuple(rows))
    ...
    def update_document(self, command) -> DesktopApiResult[DocumentDto]:
        for index, row in enumerate(self._rows):
            ...
            updated = replace(
                ...
                document_structure_id=row.document_structure_id if command.document_structure_id is None else command.document_structure_id,
                ...
                confidentiality_level=row.confidentiality_level if command.confidentiality_level is None else command.confidentiality_level,
                business_version_label=row.business_version_label if command.business_version_label is None else command.business_version_label,
        ...
        return DesktopApiResult(
            ok=False,
            error=DesktopApiError(code="document_not_found", message=f"Document '{command.document_id}' was not found.", category="not_found"),
        )
    ...
    def update_document_structure(self, command) -> DesktopApiResult[DocumentStructureDto]:
        for index, row in enumerate(self._structure_rows):
            ...
            updated = replace(
                ...
                description=row.description if command.description is None else command.description,
                parent_structure_id=row.parent_structure_id if command.parent_structure_id is None else command.parent_structure_id,
                object_scope=row.object_scope if command.object_scope is None else command.object_scope,
                default_document_type=row.default_document_type if command.default_document_type is None else command.default_document_type,
        ...
        return DesktopApiResult(
            ok=False,
            error=DesktopApiError(code="document_structure_not_found", message=f"Document structure '{command.structure_id}' was not found.", category="not_found"),
        )
    ...
    def remove_link(self, link_id: str) -> DesktopApiResult[None]:
        ...
        if len(self._link_rows) == original_count:
            return DesktopApiResult(
                ok=False,
                error=DesktopApiError(code="document_link_not_found", message=f"Document link '{link_id}' was not found.", category="not_found"),
            )
        return DesktopApiResult(ok=True, data=None)


class FakePlatformAccessApi:
    def __init__(self, *, scope_types: tuple[ScopeTypeChoiceDto, ...], scope_targets: dict[str, tuple[ScopeTargetDto, ...]], grants: tuple[ScopedAccessGrantDto, ...]) -> None:
    ...
    def list_scope_grants(self, *, scope_type: str, scope_id: str) -> DesktopApiResult[tuple[ScopedAccessGrantDto, ...]]:
        rows = [row for row in self._grants if row.scope_type == scope_type and row.scope_id == scope_id]
        return DesktopApiResult(ok=True, data=tuple(rows))

    def assign_scope_grant(self, command) -> DesktopApiResult[ScopedAccessGrantDto]:
        for index, row in enumerate(self._grants):
            if row.scope_type == command.scope_type and row.scope_id == command.scope_id and row.user_id == command.user_id:
    ...
    def remove_scope_grant(self, command) -> DesktopApiResult[None]:
        ...
        self._grants = [
            row for row in self._grants
            if not (row.scope_type == command.scope_type and row.scope_id == command.scope_id and row.user_id == command.user_id)
        ]
        if len(self._grants) == original_count:
            return DesktopApiResult(
                ok=False,
                error=DesktopApiError(code="grant_not_found", message="Scope grant was not found.", category="not_found"),
            )
        return DesktopApiResult(ok=True, data=None)


class FakePlatformApprovalApi:
    ...
    def _update_status(self, request_id: str, status: ApprovalStatus, note: str | None) -> DesktopApiResult[ApprovalRequestDto]:
        ...
        return DesktopApiResult(
            ok=False,
            error=DesktopApiError(code="request_not_found", message=f"Approval request '{request_id}' was not found.", category="not_found"),
        )


class FakePlatformEnterpriseAuditApi:
...
class FakePlatformSupportApi:
    ...
    def save_settings(self, command, *, trace_id: str | None = None) -> DesktopApiResult[SupportSettingsDto]:
        ...
        self._append_event(trace_id or "inc-support-1", "support.update.settings_saved", "Update settings saved.")
        return DesktopApiResult(ok=True, data=self._settings)
    ...
    def list_activity(self, *, trace_id: str | None = None, limit: int = 12) -> DesktopApiResult[tuple[SupportEventDto, ...]]:
    ...
    def export_diagnostics(self, *, incident_id: str) -> DesktopApiResult[SupportBundleDto]:
        return self.export_diagnostics_to(incident_id=incident_id, output_path=f"C:/pm/data/pm_diagnostics_{incident_id}.zip")

    def export_diagnostics_to(self, *, incident_id: str, output_path: str) -> DesktopApiResult[SupportBundleDto]:
...
def build_connected_platform_registry() -> SimpleNamespace:
    runtime_api = FakePlatformRuntimeApi()
    site_rows = (
        SiteDto(id="site-1", organization_id="org-1", site_code="BER", name="Berlin Campus", description="Primary operating site", country="DE", region="Berlin", city="Berlin", address_line_1="Street 1", address_line_2="", postal_code="10115", timezone="Europe/Berlin", currency_code="EUR", site_type="office", status="active", default_calendar_id="cal-1", default_language="en", is_active=True, notes="", version=1),
    ...
    user_rows = (
        ...
        UserDto(id="user-2", username="grace", display_name="Grace Hopper", email="grace@example.com", identity_provider=None, federated_subject=None, is_active=False, failed_login_attempts=2, locked_until=datetime(2026, 4, 24, 9, 0, 0), last_login_at=None, last_login_auth_method=None, last_login_device_label=None, session_expires_at=datetime(2026, 4, 24, 12, 0, 0), session_timeout_minutes_override=None, must_change_password=True, version=1, role_names=("viewer",)),
    )
    user_api = FakePlatformUserApi(user_rows, role_rows)
    access_scope_types = (
        ...
        ScopeTypeChoiceDto(label="Storeroom", value="storeroom", enabled=False, supporting_text="Inventory & Procurement is disabled. Enable it before managing storeroom access."),
    )
    access_scope_targets = {
        "project": (ScopeTargetDto(id="project-1", label="Project Apollo", scope_type="project"),),
        "site": (ScopeTargetDto(id="site-1", label="BER - Berlin Campus", scope_type="site"),),
        "storeroom": (),
    }
    access_grants = (
        ScopedAccessGrantDto(id="grant-1", scope_type="project", scope_id="project-1", user_id="user-1", scope_role="manager", permission_codes=("project.manage", "project.read"), created_at=datetime(2026, 4, 24, 8, 15, 0)),
        ScopedAccessGrantDto(id="grant-2", scope_type="site", scope_id="site-1", user_id="user-2", scope_role="viewer", permission_codes=("site.read",), created_at=datetime(2026, 4, 24, 8, 45, 0)),
    )
    access_api = FakePlatformAccessApi(scope_types=access_scope_types, scope_targets=access_scope_targets, grants=access_grants)
    document_structure_rows = (
        DocumentStructureDto(id="structure-1", organization_id="org-1", structure_code="POL", name="Policies", description="Policy documents", parent_structure_id=None, object_scope="GENERAL", default_document_type="POLICY", sort_order=1, is_active=True, notes="", version=1),
        DocumentStructureDto(id="structure-2", organization_id="org-1", structure_code="PROC", name="Procedures", description="Procedure documents", parent_structure_id=None, object_scope="GENERAL", default_document_type="PROCEDURE", sort_order=2, is_active=True, notes="", version=1),
    )
    document_rows = (
        DocumentDto(id="doc-1", organization_id="org-1", document_code="DOC-001", title="Governance Charter", document_type="GENERAL", document_structure_id="structure-1", storage_kind="FILE_PATH", storage_uri="/docs/doc-1.pdf", file_name="doc-1.pdf", mime_type="application/pdf", source_system="desktop", uploaded_at=None, uploaded_by_user_id=None, effective_date=None, review_date=None, confidentiality_level="internal", business_version_label="1.0", is_current=True, notes="", is_active=True, version=1),
        DocumentDto(id="doc-2", organization_id="org-1", document_code="DOC-002", title="Archived Procedure", document_type="GENERAL", document_structure_id="structure-2", storage_kind="FILE_PATH", storage_uri="/docs/doc-2.pdf", file_name="doc-2.pdf", mime_type="application/pdf", source_system="desktop", uploaded_at=None, uploaded_by_user_id=None, effective_date=None, review_date=None, confidentiality_level="internal", business_version_label="0.9", is_current=False, notes="", is_active=True, version=1),
    )
    document_link_rows = (
    ...
    approval_rows = (
        ApprovalRequestDto(id="approval-1", request_type="budget_change", entity_type="project", entity_id="project-1", project_id="project-1", status=ApprovalStatus.PENDING, module_label="Project Management", context_label="Project Apollo", display_label="Change Budget", requested_by_username="ada", requested_at=datetime(2026, 4, 24, 7, 30, 0)),
        ApprovalRequestDto(id="approval-2", request_type="baseline_publish", entity_type="project_baseline", entity_id="baseline-1", project_id="project-1", status=ApprovalStatus.APPROVED, module_label="Project Management", context_label="Project Apollo", display_label="Publish Baseline", requested_by_username="grace", requested_at=datetime(2026, 4, 24, 8, 0, 0)),
    ...
    return SimpleNamespace(
        ...
        platform_approval=FakePlatformApprovalApi(approval_rows),
        platform_enterprise_audit=FakePlatformEnterpriseAuditApi(audit_rows),
        platform_support=FakePlatformSupportApi(),
    )
