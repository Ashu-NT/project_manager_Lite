...
def test_inventory_repositories_hide_cross_organization_rows(services) -> None:
    ...
    current_category_ids = {
        row.id
        for row in category_repo.list_for_organization(seeded["current_org_id"], active_only=None)
    }
    current_item_ids = {
        row.id
        for row in item_repo.list_for_organization(seeded["current_org_id"], active_only=None)
    }
    current_storeroom_ids = {
        row.id
        for row in storeroom_repo.list_for_organization(seeded["current_org_id"], active_only=None)
    }
    current_balance_ids = {
    ...
    current_transaction_ids = {
        row.id
        for row in transaction_repo.list_for_organization(seeded["current_org_id"], limit=200)
    }
    current_location_ids = {
        row.id
        for row in location_repo.list_for_organization(seeded["current_org_id"], active_only=None)
    }
    current_policy_ids = {
        row.id
        for row in policy_repo.list_for_organization(seeded["current_org_id"], active_only=None)
    }
    current_cycle_count_ids = {
        row.id
        for row in cycle_count_repo.list_for_organization(seeded["current_org_id"], limit=200)
    }
