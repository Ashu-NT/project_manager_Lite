...
from core.services.import_service import DataImportService, ImportFieldSpec, ImportPreview
...
class ImportWizardDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        intro = QLabel(
            "Preview the import before commit, map source columns to app fields, and import only the rows that validate."
        )
        ...
        for btn in (self.btn_browse, self.btn_preview, self.btn_import, self.btn_close):
    ...
    def _entity_type(self) -> str:
        return str(self.type_combo.currentData() or "projects")
    ...
    def _reload_mapping_fields(self) -> None:
        ...
        schema = self._data_import_service.get_import_schema(self._entity_type())
    ...
    def _run_preview(self) -> None:
        path = self._selected_path()
        if path is None:
            QMessageBox.information(self, "Import Wizard", "Choose a CSV file first.")
            return
        try:
            preview = self._data_import_service.preview_csv(
                self._entity_type(),
                path,
    ...
    def _render_preview(self, preview: ImportPreview) -> None:
        ...
        for row_idx, row in enumerate(preview.rows):
            ...
            for col, value in enumerate(values):
                ...
                self.preview_table.setItem(row_idx, col, item)
    ...
    def _run_import(self) -> None:
        ...
        if self._current_preview is None:
            ...
            if self._current_preview is None:
                return
        try:
            summary = self._data_import_service.import_csv(
                self._entity_type(),
                path,
                column_mapping=self._mapping_payload(),
            )
        ...
        QMessageBox.information(self, "Import Wizard", "\n".join(details))
        self.accept()


__all__ = ["ImportWizardDialog"]
